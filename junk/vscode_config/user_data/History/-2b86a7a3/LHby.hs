...
test2 = testCase "Funded cluster starts up funded" $ do
  withCluster def distr $ \cenv keys -> do
    res <- zipWithM (\ds1 key -> right (eqDistr ds1) <$> utxosAtAddress cenv key) distr (cardanoMainnetAddress <$> keys)
    case sequence_ res of
      Left err -> assertFailure $ "Failed to query the cluster node for utxos: " <> show err
      Left _ -> assertFailure $ "Queried utxo distribution is incorrect."
      Right _ -> pure ()
