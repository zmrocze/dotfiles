  ...
  outputs = inputs@{ self, nixpkgs, haskell-nix, plutip, cardano-transaction-lib, ... }:
    let
      ...
      onchain = rec {
        ...
        script-exporter = system:
          let
            pkgs' = nixpkgsFor' system;
            exporter = ((projectFor system).flake { }).packages."cardano-mlabs-template-onchain:exe:exporter";
          in
      ...
      offchain = {
        projectFor = system:
          let
            pkgs = nixpkgsFor system;
            exporter = ((onchain.projectFor system).flake { }).packages."cardano-mlabs-template-onchain:exe:exporter";
          in
          pkgs.purescriptProject {
            inherit pkgs;
            projectName = "cardano-mlabs-template-project";
    ...
    in
    {
      ...
      checks = perSystem (system:
        ...
        // {
          cardano-mlabs-template = self.offchain.project.${system}.runPlutipTest { testMain = "Test"; };
        }
      );
