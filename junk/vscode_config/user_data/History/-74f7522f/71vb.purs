...
import Ctl.Internal.QueryM.ServerConfig (Host, ServerConfig, defaultDatumCacheWsConfig, defaultOgmiosWsConfig, defaultServerConfig, mkHttpUrl, mkOgmiosDatumCacheWsUrl, mkServerUrl, mkWsUrl) as ExportServerConfig
...
import Ctl.Internal.Wallet (Cip30Connection, Cip30Wallet, Wallet(Gero, Flint, Nami, Eternl, Lode, KeyWallet), mkEternlWalletAff, mkFlintWalletAff, mkGeroWalletAff, mkKeyWallet, mkLodeWalletAff, mkNamiWalletAff)
    ...
    Just config -> case traverse plutusDataToAeson args of
      ...
      Just ps -> do
        let
          ...
          url :: String
          url = let x = mkHttpUrl config <> "/apply-args" in trace x (\_ -> x)
