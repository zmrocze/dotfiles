    ...
    forM_ (Map.assocs $ txMintingScripts tx) $ \(mph, red) -> do
        ...
        ctx <-
            if lang == PlutusV1 then do
                ...
                pure $ Context $ PV1.toBuiltinData $ PV1.ScriptContext { PV1.scriptContextPurpose = PV1.Minting cs, PV1.scriptContextTxInfo = txInfo }
            else do
                txInfo <- mkPV2TxInfo tx
                pure $ Context $ PV2.toBuiltinData $ PV2.ScriptContext { PV2.scriptContextPurpose = PV2.Minting cs, PV2.scriptContextTxInfo = txInfo }
