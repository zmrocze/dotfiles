...
import Ledger (Signature, Tx (txData), Validator (Validator))
-- import Ledger.Constraints (ScriptLookups(ScriptLookups))
...
-- | Constraints on transactions that want to spend script outputs
data TxConstraint1 =
      MustHashDatum1 DatumHash Datum
    -- ^ Like 'MustHashDatum', but the hash of the 'Datum' is computed automatically.
    | MustValidateIn1 POSIXTimeRange
    -- ^ The transaction's validity range must be set with the given 'POSIXTimeRange'.
    | MustBeSignedBy1 PaymentPubKeyHash
    | MustPayToOtherScript1 ValidatorHash (Maybe StakeValidatorHash) Datum Value
    

-- data BuildWith a = BuildWith

data TxConstraint1' a b where
    MustHashDatum1' :: DatumHash -> Datum -> TxConstraint1' a a
    -- | Like 'MustHashDatum', but the hash of the 'Datum' is computed automatically.
    MustValidateIn1' :: POSIXTimeRange -> TxConstraint1' a a
    -- | The transaction's validity range must be set with the given 'POSIXTimeRange'.
    MustBeSignedBy1' :: PaymentPubKeyHash -> TxConstraint1' a (Signature -> a)
    
    -- | The transaction must create a transaction output with a script address.
    MustPayToOtherScript1' :: ValidatorHash -> (Maybe StakeValidatorHash) -> Datum -> Value -> TxConstraint1' a (Validator -> a)
    

tx1'2tx1 :: TxConstraint1' a b -> TxConstraint1
tx1'2tx1 (MustHashDatum1' datumhash datum) = MustHashDatum1 datumhash datum
tx1'2tx1 (MustValidateIn1' timerange) = MustValidateIn1 timerange
tx1'2tx1 (MustBeSignedBy1' pubkeyhash) = MustBeSignedBy1 pubkeyhash

data TxConstraintEx = forall a b . TxConstraintEx (TxConstraint1' a b)

tx12tx1' :: TxConstraint1 -> TxConstraintEx
tx12tx1' (MustHashDatum1 datumhash datum) = TxConstraintEx $ MustHashDatum1' datumhash datum
tx12tx1' (MustValidateIn1 timerange) = TxConstraintEx $ MustValidateIn1' timerange
tx12tx1' (MustBeSignedBy1 pubkeyhash) = TxConstraintEx $ MustBeSignedBy1' pubkeyhash

-- a :: Signature -> Tx

buildTx :: TxConstraint1' a b -> (ScriptLookups -> a) -> (ScriptLookups -> b)
buildTx (MustHashDatum1' datumhash datum) a sl = a $ sl {slOtherData = Map.insert datumhash datum (slOtherData sl) }
buildTx (MustValidateIn1' timerange) a sl = a sl -- what to do here?
buildTx (MustBeSignedBy1' pubKeyHash) a sl = \signature -> a $ sl {  }
