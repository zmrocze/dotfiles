...
type BuildWith a b = a -> b

data TxConstraint1' a where
    MustHashDatum1' :: DatumHash -> Datum -> TxConstraint1' (BuildWith ())
    -- | Like 'MustHashDatum', but the hash of the 'Datum' is computed automatically.
    MustValidateIn1' :: POSIXTimeRange -> TxConstraint1' (BuildWith ())
    -- | The transaction's validity range must be set with the given 'POSIXTimeRange'.
    MustBeSignedBy1' :: PaymentPubKeyHash -> TxConstraint1' (BuildWith Signature)
