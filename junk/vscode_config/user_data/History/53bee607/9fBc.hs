module Spec.TestContract.LockSpendMint (lockThenSpend, mintZeroTokens) where
...
import Ledger.Value (flattenValue, tokenName, Value (Value))
import Plutus.Contract (Contract, awaitTxConfirmed, submitTx, submitTxConstraintsWith, mkTxConstraints, adjustUnbalancedTx)
...
mintZeroTokens :: PaymentPubKeyHash -> Contract () EmptySchema Text ()
mintZeroTokens pkh = do
  let 
    token = Value.singleton currencySymbol (tokenName "ff") 0

    txc = Constraints.mustMintValue token
