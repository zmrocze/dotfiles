...
Add relevant tests to increase the confidence that the implementation indeed works as specified. Tests should cover the scenarios above, covering the positive case and also failing as expected for incorrect reward calculation or unauthorised spend attempts. Tests should attempt to steal the collected fees, spending `Collector` owned _eUTxO_s to a private address.
