...
type GetApi = "query" :> QueryParam "key" Text :> Get '[PlainText] Integer
...
server var = handlePost var :<|> handleGet var
  where 
    ...
    handleGet ::  TVar CountMap -> Maybe Text -> Handler Integer
    handleGet var = maybe  $ \k -> do 
      where 
        noKeyError = throwError $ err400 { errBody = "Expected key parameter." }
