...
type GetApi = "query" :> QueryParam "key" Text :> Get '[PlainText] Text
...
server var = handlePost var :<|> handleGet var
  where 
    ...
    handleGet ::  TVar CountMap -> Maybe Text -> Handler Text
    handleGet var = maybe noKey $ \k -> liftIO $ pack . getCount k <$> readTVarIO var
