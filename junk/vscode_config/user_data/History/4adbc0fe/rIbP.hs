...
import Cardano.Api (SerialiseAsCBOR, ToCBOR, StakeCredential, StakePoolParameters, EpochNo, GenesisKey, Hash, GenesisDelegateKey, MIRTarget, SerialiseAsRawBytes (deserialiseFromRawBytes), HasTypeProxy (proxyToAsType))
import Cardano.Api.Shelley (PoolId, VrfKey)
import Cardano.Ledger.Shelley.API (MIRPot)
import Data.Data (Typeable, Proxy (Proxy), typeRep, typeRepTyCon)
...
-- deriving via UsingRawBytesHex instance ToJSON (Hash )