...
# at https://hydra.iohk.io/build/17265750.
...
if ! [ -d "$inputDir" ]; then
    info "fetching Git input ‘plutus-apps’ from ‘https://github.com/input-output-hk/plutus-apps.git’ (commit bc1724c5e88197d3416fdf33190a52e916414993)..."
    requireCommand git
    inputDirTmp="$inputDir.tmp"
    rm -rf "$inputDirTmp"
    mkdir -p "$inputDirTmp"
    git clone 'https://github.com/input-output-hk/plutus-apps.git' "$inputDirTmp"
    (cd "$inputDirTmp" && git checkout 'bc1724c5e88197d3416fdf33190a52e916414993')
    revCount="$(cd "$inputDirTmp" && (git rev-list 'bc1724c5e88197d3416fdf33190a52e916414993' | wc -l))"
