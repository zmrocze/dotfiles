...
import Ledger.Tx (RedeemerPtr, ScriptTag, Tx, TxId, TxIn, TxInType, TxOut, TxOutRef, TxInputType, TxInput, Withdrawal, Certificate)
...
import Test.QuickCheck.Arbitrary.Generic (Arbitrary, arbitrary, genericArbitrary, genericShrink, shrink, oneOf)
...
instance Arbitrary Ledger.Script where
    arbitrary = oneOf [
          pure Ledger.fromCompiledCode $$(PlutusTx.compile [|| (\_ _ -> ()) ||])
        , pure Ledger.fromCompiledCode $$(PlutusTx.compile [|| (\_ _ _ -> ()) ||])
        ]
