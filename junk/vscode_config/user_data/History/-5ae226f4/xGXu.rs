...
use pallas_codec::utils::{AnyUInt, Bytes, PlutusBytes, Int, KeyValuePairs};
...
impl ToPlutusData for PlutusBytes {
    fn to_plutus_data(&self) -> PlutusData {
...
impl ToPlutusData for Bytes {
    fn to_plutus_data(&self) -> PlutusData {
        PlutusData::BoundedBytes(PlutusBytes::from(self.deref().clone()))
    }
}
    ...
    fn to_plutus_data(&self) -> PlutusData {
        match self {
            Value::Coin(coin) => PlutusData::Map(KeyValuePairs::Def(vec![(
                PlutusBytes::from(vec![]).to_plutus_data(),
