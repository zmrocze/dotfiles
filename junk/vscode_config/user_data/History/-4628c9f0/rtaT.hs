...
data TxInType =
      ConsumeScriptAddress !ValidatorHash !DatumHash -- ^ A transaction input that consumes a script address with the referenced validator and datum.
...
instance Pretty TxIn where
    pretty TxIn{txInRef,txInType} =
        hang 2 $ vsep ["-" <+> pretty txInRef, rest]
...
inScripts :: TxIn -> Maybe (ValidatorHash, DatumHash)
inScripts TxIn{ txInType = t } = case t of
    Just (ConsumeScriptAddress vh dh) -> Just (vh, dh)
    _                                 -> Nothing
...
scriptTxIn :: TxOutRef -> ValidatorHash -> DatumHash -> TxIn
