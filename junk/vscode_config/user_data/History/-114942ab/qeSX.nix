{
  ...
  outputs = inputs@{ self, nixpkgs, cardano-transaction-lib, tooling, pre-commit-hooks, ... }:
    let
      ...
      # only partial, combine with mlabs-tooling pre-commit
      pre-commit-check = let
        ...
        in pre-commit-hooks.lib.${system}.run {
          src = ./.;
          hooks = {
            nixpkgs-fmt = excludeSpago;
            statix = excludeSpago;
            deadnix = excludeSpago;
            prettier.enable = true;
            purs-tidy.enable = true;
          };
        };
      shell-hook = 
        onchain.devShell.shellHook
    ...
    in
    {
      ...
      packages = onchain.packages;

      checks = onchain.checks
        // (perSystem (system:
        {
      ...
      devShells = let 
        overrideShellHook = shell: shell.overrideAttrs (finalAttrs: finalAttrs // {
              shellHook = finalAttrs.shellHook + config.pre-commit.installationScript;
            }); (perSystem (system: {
        onchain = onchain.devShells.${system}.default;
        offchain = self.offchain.${system}.devShell;
        tooling = nixpkgs.legacyPackages.${system}.mkShell {
          inherit shell-hook;
        };
      }));
