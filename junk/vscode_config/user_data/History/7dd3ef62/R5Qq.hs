...
-- | Create a wallet with the given amounts of lovelace, containing staking keys
--  and after contract execution
-- compare the values at the wallet address with the given ordering and value.

--
-- @since 
initLovelaceAndStakingAssertValueWith :: [Positive] -> ValueOrdering -> Value -> TestWallets
initLovelaceAndStakingAssertValueWith initial ord expect = TestWallets $ TestWallet initial (Just (ord, expect)) True :| []
