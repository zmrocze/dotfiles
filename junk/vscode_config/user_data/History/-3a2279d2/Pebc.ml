...
module Make (Toolbox : Iface.TYPECHECKER_TOOLBOX) = struct
  ...
  let errorReporter () = 
    logf "Failed ups";
    ErrorReporter.report_bad_number_of_arguments ~loc:(Location {line=0; column=69; file="helloworld.hs"}) ~expected:420 ~actual:420;
    ErrorReporter.fail ()
  ...
  in begin check_uniqueness (List.mapi (fun x y -> (x,y)) names) ~name_of:((fun x -> Some x) << snd) ~loc_of:fst ~report_error:(fun x y -> errorReporter ());
  ...
  module TopEnv = struct
    ...
    let declare_type top_env ~loc id =
      ...
      match IdMap.find_opt id top_env with 
        ...
        | Some (ST_Function _) -> errorReporter ()
    ...
    let declare_function top_env ~loc id param_types return_type =
       ...
       *)
       ...
       | Some (ST_AbstractType _) -> errorReporter ()
       | Some (ST_TypeDef {loc; body=body1}) -> errorReporter ()
  ...
  and _infer_expression env e =
    ...
    | EXPR_Call { callee; arguments } -> begin match (Env.lookup_global env callee) with 
      | Some (ST_Function {loc; param_types; return_type = Some ret_type}) ->
        if for_all2 (check_expression env) arguments param_types 
        then Some ret_type else None
      | _ -> None
      end
    ...
    | EXPR_Record flds -> let  
      mflds = List.map (
        fun x -> x.data 
        |> function FieldExpr {name=nm; value=ex} ->
           Option.map (fun x -> (nm, x)) (infer_expression env ex)) 
        flds in 
      let tpflds = List.fold_left (fun ml mfld -> Option.bind ml (fun l -> Option.bind mfld (fun fld -> Some (fld :: l)))) (Some []) mflds
  ...
  and check_expression env e expected =
    ...
    | EXPR_Binop { op = BINOP_Add; lhs; rhs }, _ -> errorReporter ()
    ...
    | EXPR_Record fields, TP_Record ftypes -> 
      let _ = throwBool errorReporter @@ for_all2 (fun x (nm, tp) -> 
    ...
    | ( ( EXPR_Id _ | EXPR_Int _ | EXPR_Char _ | EXPR_String _ | EXPR_Bool _
        ...
        | None -> errorReporter () (* why this wasn't an error? *)
        | Some actual ->
  ...
  let check_lvalue env lv = match 
    ...
    | LVALUE_Index { expr; index } -> let 
      _ = throwBool errorReporter @@ check_expression env index TP_Int in
  ...
  let rec check_statement env stmt =
    ...
    | STMT_Assign { lhs; rhs} -> 
      ...
      let _ = throwBool errorReporter @@ check_expression env rhs tp in
      env
      ...
      | Some (ST_Function {return_type=None; param_types=param_types; loc}) -> 
        begin match forM (infer_expression env) arguments with 
          ...
          | Some ts -> if for_all2 eq_normal_type ts param_types then 
            env else errorReporter ()
      end
      | Some _ -> errorReporter ()
    end
    | STMT_VarDecl { var = { data = VarDecl { id; tp }; loc; _ }; init } ->
        ...
        logf "hello2";
        (match init with None -> () | Some e -> throwBool errorReporter @@ check_expression env e tp);
        logf "hello3";
        env
    | STMT_ArrayDecl { id; tp=tp_n; sizes } -> 
      ...
      (List.iter (fun e -> throwBool errorReporter @@ check_expression env e TP_Int) sizes);
        env
    | STMT_If { cond; then_branch; else_branch } -> begin
        throwBool errorReporter @@ check_expression env cond TP_Bool;
    ...
    | STMT_Return rv -> begin match rv with
      | None -> (match Env.return_type env with | None -> env | Some _ -> errorReporter ())
      | Some v -> (match Env.return_type env with 
        | None -> errorReporter ()
        | Some t -> if check_expression env v t then env else errorReporter ()
        )
    end 
    | STMT_Block block ->
