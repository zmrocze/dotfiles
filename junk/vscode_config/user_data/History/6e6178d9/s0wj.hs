...
validateLiquidateCDP
  ...
  ctx =
    traceIfFalse "Invalid input being validated" (ownRef == cdpRef)
      ...
      && traceIfFalse "Only burning tokens" (txMint `Value.lt` mempty)
      ...
      && ( if isPartialLiquidation
             then
               ...
               traceIfFalse
                 "Expected burn all cdp auth tokens in cdp input"
                 ( mintedCdpToken
                     == negate (V2.assetClassValueOf cdpInputValue cdpAuthToken)
                 )
                 && traceIfFalse
                   "Stability pool output value is incorrect"
                   ( spOutputValue
    ...
    where
      ...
      mintedCdpToken :: Integer
      mintedCdpToken = Value.assetClassValueOf txMint cdpAuthToken
