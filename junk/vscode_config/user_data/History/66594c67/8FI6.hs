...
import Prettyprinter (Pretty(pretty), layoutPretty, defaultLayoutOptions, Doc)
...
shouldSucceed =
  Predicate
    ...
    (mappend "But it didn't.\nResult: " . renderString . layoutPretty defaultLayoutOptions . pretty)
    isSuccessful

prettyExecutionResult :: ExecutionResult e w a -> Doc ann
prettyExecutionResult (ExecutionResult {outcome, state}) = vsep ["Execution result {",
  "outcome:" <+> 
