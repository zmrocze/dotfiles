...
import Ledger.Contexts
    ( TxOut(txOutAddress), TxId, TxOutRef(TxOutRef) )
...
import Ledger.Tx
    ( txId,
      SomeCardanoApiTx,
      TxIn,
      Tx(Tx, txInputs, txOutputs, txCollateral, txValidRange, txData,
         txScripts, txMintingScripts, txWithdrawals, txCertificates),
      Certificate(certificateRedeemer),
      Withdrawal(withdrawalRedeemer),
      TxInput(txInputType),
      TxInputType(TxConsumeScriptAddress),
