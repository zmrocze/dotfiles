...
import Ledger.Tx (Certificate (certificateRedeemer), TxId, TxIn, TxInput (txInputType),
                  TxInputType (TxConsumeScriptAddress), TxOut (txOutAddress), TxOutRef (TxOutRef),
                  Withdrawal (withdrawalRedeemer), fillTxInputWitnesses, txId, SomeCardanoApiTx)
import Prettyprinter ( (<+>), vsep, viaShow, nest, hang, colon, braces, Pretty(pretty) )
