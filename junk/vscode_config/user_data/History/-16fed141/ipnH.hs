...
startClusterHandler
  ...
  StartClusterRequest {keysToGenerate} = interpret $ do
    ...
    where
      setup :: ReaderT ClusterEnv IO (ClusterEnv, [BpiWallet k])
      setup = do
        env <- ask
        wallets <- do
          for (zip [0..] keysToGenerate) $ \(idx, lovelaceAmounts) -> do
            addSomeWallet (testWallet' (fromInteger . unLovelace <$> lovelaceAmounts) Nothing ())
