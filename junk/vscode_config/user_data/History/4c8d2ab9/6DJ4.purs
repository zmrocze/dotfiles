...
main = launchAff_ $ do
  ...
  case args of 
    [_, filepath] -> do 
      index <- readIndex filepath
      if validateIndex index then 
        liftEffect $ print $ importAllScripts index
      else 
        throwError $ error "Invalid index file: script names should be identifiers and script hashes be hex encoded hashes." 
    _ -> throwError $ error "Provide the path to index file as a single command line argument."
...
jsModule = importAllScriptsJS
psModule = importAllScriptsPS

importAllScriptsJS :: Array (Tuple String String) -> String
importAllScriptsJS index = joinWith "\n" $
  [ifBrowserRuntime "" nodeImports]
  <> map (uncurry importScript) index
...
require_node script_hash = "read_script(\""<>filename<>"\");"
  where filename = script_hash <> ".plutus"
...
importAllScriptsPS :: Array String -> String
importAllScriptsPS script_names = joinWith "\n" $
  map importScript script_names
