...
import Ledger.Tx (CardanoTx (..), txId, updateUtxoCollateral, TxInput (TxInput, txInputRef))
...
import Plutus.V1.Ledger.Tx
    ( TxOut(txOutDatumHash, txOutAddress, txOutValue),
      TxOutRef,
      collateralInputs,
      datumWitnesses,
      inputs,
      signatures,
      Tx(Tx, txOutputs, txMint, txFee, txValidRange),
      TxIn(txInType, txInRef),
