...
import Ledger.Tx (CardanoTx (..), txId, updateUtxoCollateral, TxInput (TxInput, txInputRef), TxOut, Tx(..))
...
import Plutus.V1.Ledger.Tx (TxOutRef, TxIn (txInType), TxInType (ConsumeScriptAddress, ConsumePublicKeyAddress), txOutDatumHash, txOutAddress)
