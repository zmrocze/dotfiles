...
withContract $ \_ -> do
  ...
  case addr of 
    Address (PubKeyCredential ourPkh) (Just (StakingHash )) -> logInfo "This is the address we will get."
    
```

 Note that using base address for contract's own address
If it was initialized with `BaseTag`, you should likely use `Contract.ownAddresses` instead of `ownPaymentPubKeyHash` as contract address will be a base address (with staking keys).
