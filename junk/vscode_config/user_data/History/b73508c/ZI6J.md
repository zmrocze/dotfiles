...
* `PaymentPubKeyHash` of wallet `initAda [100]` will be "own" `PaymentPubKeyHash` for contract executed it test case. If it were a base wallet, you should use `Contract.ownAddresses` and take first address instead of `ownPaymentPubKeyHash` as contract address will be base address (with staking keys)
