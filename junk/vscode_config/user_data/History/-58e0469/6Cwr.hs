...
import Ledger (ScriptPurpose(..), Redeemer, Credential, DCert)
...
scriptPurposeToExportRedeemer (P.Spending ref)     = SpendingRedeemer {redeemerOutRef = ref}
scriptPurposeToExportRedeemer (P.Minting cs)       = MintingRedeemer {redeemerPolicyId = currencyMpsHash cs}
scriptPurposeToExportRedeemer (P.Rewarding cred)   = RewardingRedeemer {redeemerStakingCredential = cred}
scriptPurposeToExportRedeemer (P.Certifying dcert) = CertifyingRedeemer {redeemerDCert = dcert}
