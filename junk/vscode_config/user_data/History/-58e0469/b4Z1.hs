...
import Ledger (Redeemer, Credential, DCert, txRedeemers)
...
export params utx =
    ...
    in ExportTx
        ...
        <*> mkInputs (P.pNetworkId params) unBalancedTxUtxoIndex
...
scriptPurposeToExportRedeemer (Ledger.Spending ref)     = SpendingRedeemer {redeemerOutRef = ref}
scriptPurposeToExportRedeemer (Ledger.Minting cs)       = MintingRedeemer {redeemerPolicyId = currencyMpsHash cs}
scriptPurposeToExportRedeemer (Ledger.Rewarding cred)   = RewardingRedeemer {redeemerStakingCredential = cred}
scriptPurposeToExportRedeemer (Ledger.Certifying dcert) = CertifyingRedeemer {redeemerDCert = dcert}
