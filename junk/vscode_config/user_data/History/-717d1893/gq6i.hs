...
import Ledger (
  ...
  TxOutRef,
  getCardanoTxId,
  scriptHashAddress, fromCompiledCode
 )
...
import Cardano.Address.Script (serializeScript)
import Codec.Serialise
import Cardano.Api (writeFileTextEnvelope, PlutusScriptV1, PlutusScript, SerialiseAsCBOR (deserialiseFromCBOR), AsType (AsScript, AsPlutusScriptV1))
...
-- a :: PlutusTx.CompiledCode (() -> () -> ScriptContext -> Bool)
-- a = $$(PlutusTx.compile [||mkValidator||])
...
script = fromCompiledCode alwaysSucceedsCompiled

-- serialized_script = serialiseCompiledCode _

Right x = deserialiseFromCBOR (AsScript AsPlutusScriptV1) $ B.toStrict $ serialise script

b = writeFileTextEnvelope @(PlutusScript PlutusScriptV1) "always-succeeds.plutus" (Just "My little script") x
