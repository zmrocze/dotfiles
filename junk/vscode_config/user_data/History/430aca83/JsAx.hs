...
import Ledger (
  ...
  Value,
  ciTxOutValue,
  getCardanoTxId, Address (Address)
 )
...
payToPubKeyAddress (Address crd stake) amt = do
  ...
  let constr = maybe (Constraints.mustPayToPubKey (PaymentPubKeyHash pkh) (Ada.lovelaceValueOf amt)) 
    (\spkh -> Constraints.mustPayToPubKeyAddress (PaymentPubKeyHash pkh) spkh (Ada.lovelaceValueOf amt)) mspkh

  tx <- submitTx (Constraints.mustPayToPubKey (PaymentPubKeyHash pkh) (Ada.lovelaceValueOf amt))
  _ <- Contract.awaitTxConfirmed (getCardanoTxId tx)
  pure tx
