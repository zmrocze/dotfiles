{
  ...
  outputs = inputs@{ self, nixpkgs, cardano-transaction-lib, tooling, ... }:
    ...
    in
    {
      ...
      packages = onchain.packages // offchain.packages ;
      inherit onchain;
      ...
      checks = onchain.checks
        ...
        // (perSystem (system:
        {
          plutip-tests = self.offchain.${system}.runPlutipTest { testMain = "Test"; };
          # TODO e2e-tests = self.offchain.${system}.runE2ETest { testMain = ""; };
        }
      ));
