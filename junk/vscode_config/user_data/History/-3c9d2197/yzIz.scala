...
sealed abstract class NNFedGFFormula[T, X] extends Pretty {
  ...
  def toGFormula(): GFFormula[X] = this match
    ...
    case NNFAnd(tag, left, right) => And(left, right)
    case NNFOr(tag, left, right) => Or(left, right)
    case NNFForall(tag, variables, guard, sub) => Forall(variables, guard, sub)
    case NNFExist(tag, variables, guard, sub) => Exist(variables, guard, sub)
