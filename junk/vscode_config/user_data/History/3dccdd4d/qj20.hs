...
import Prelude
import Test.Plutip.Internal.BotPlutusInterface.Types (WalletInfo(EnterpriseInfo), WalletTag (EnterpriseTag), WalletType (Enterprise))
...
runAdjustTest =
  assertExecution
    ...
    (initAda (EnterpriseTag 0 :: WalletTag 'Enterprise Int) [1000] <> initAda (EnterpriseTag 1 :: WalletTag 'Enterprise Int) [1000])
    (withContract $ \wl -> do 
      EnterpriseInfo pkh <- lookupWallet wl (EnterpriseTag 1 :: WalletTag 'Enterprise Int)
      _ $ adjustTx' [pkh])
    [ shouldSucceed
    , yieldSatisfies
