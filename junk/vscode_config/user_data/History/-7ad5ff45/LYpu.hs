...
import Test.Plutip.Tools.CardanoApi (awaitWalletFunded, AwaitWalletFundedError(AwaitingCapiError, AwaitingTimeoutError), CardanoApiError(SomeError))
import Types (
  AppM,
...
startClusterHandler
  ...
  StartClusterRequest {keysToGenerate} = interpret $ do
    ...
    res <- waitForFundingTxs clusterEnv wallets
    ...
    where
      ...
      waitForFundingTxs clusterEnv wallets = do
        ...
        return $ firstJust (\case
          Left (AwaitingCapiError (SomeError e)) -> Just e
