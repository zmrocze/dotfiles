...
import Ledger (OnChainTx (..), ScriptTag (Cert, Mint, Reward), SomeCardanoApiTx (SomeTx), Tx (..), TxOutRef (..),
               onCardanoTx, txCertifyingRedeemers, txId, txMintingRedeemers, txRewardingRedeemers, txSpendingRedeemers, TxInput (txInputType))
import Ledger.Address (Address)
import Ledger.Scripts (Redeemer, RedeemerHash)
import Ledger.Tx (fillTxInputWitnesses, TxInputType (TxConsumeScriptAddress))
...
calculateRedeemerPointers tx = spends <> rewards <> mints <> certs
    ...
    where
        ...
        spends  = Map.fromList $ zipWith (\n (_, rd) -> (RedeemerPtr Spend n, rd)) [0..]  $ sort scriptInputs
        ...
        scriptInputs = mapMaybe (uncurry getRd) $ zip [0..] $ fmap txInputType $ sort $ txInputs tx
        -- inputsRedeemers = fmap snd $ Map.assocs $ txSpendingRedeemers tx
        getRd n = \case
            TxConsumeScriptAddress _ rd _ _ -> Just (n, rd)
            _ -> Nothing