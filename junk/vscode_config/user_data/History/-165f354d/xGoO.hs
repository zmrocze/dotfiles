...
import Plutus.V1.Ledger.Api (StakingCredential(..), Credential (..), ToData (toBuiltinData), ScriptPurpose (Spending))
...
scriptCred = ScriptCredential "valihash"
...
txinfo =  TxInfo
    ...
    , txInfoData        = [("datahash", Datum (toBuiltinData [1 :: Integer, 2]))]
    , txInfoId          = TxId "txid"
    -- ^ Hash of the pending transaction (excluding witnesses)
    }

context = ScriptContext txinfo (Spending (TxOutRef (TxId "txid") 0))

context_data = toData context  