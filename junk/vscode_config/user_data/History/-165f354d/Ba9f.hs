...
import Ledger (ScriptContext(ScriptContext), unitRedeemer, TxInfo (..), TxInInfo (TxInInfo), TxOutRef (..), TxId (TxId), TxOut (TxOut), Address (..), always, interval, fromMilliSeconds)
...
import Plutus.V1.Ledger.Api (StakingCredential(..), Credential (..))
...
timerange1 = interval (fromMilliSeconds 100) (fromMilliSeconds 200)
timerange2 = always
txinfo =  TxInfo
