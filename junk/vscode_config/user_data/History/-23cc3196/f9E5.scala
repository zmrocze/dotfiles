...
case class AndF[A, T, X](tag : T, left : A, right : A)
case class OrF[A, T, X](tag : T, left : A, right : A)
case class ForallF[A, T, X](tag : T, variables : List[X], guard : AtomicFormula[X], sub : A)
case class ExistF[A, T, X](tag : T, variables : List[X], guard : AtomicFormula[X], sub : A)

rec type And1 = AndF[And1, Int, Char]
