...
sealed trait GFFormula[X] extends Pretty {
    ...
    override def prettyPrio(prio : Int): String = 
        ...
        this match {
        case Literal(tag, sign, atom) => (if sign then "" else "¬") + atom.pretty()
        case And(tag, left, right) => parens ( 6 < prio, 
            left.prettyPrio(6) + " ∧ " + right.prettyPrio(6))
        case Or(tag, left, right) => parens ( 4 < prio, 
            left.prettyPrio(4) + " v " + right.prettyPrio(4))
        case Forall(tag, variables, guard, sub) => parens ( 0 < prio,
            "∀ (" + commaInterleaved(variables.map(_.toString)) + ") " + guard.pretty() + " → " + sub.prettyPrio(2))
        case Exist(tag, variables, guard, sub) => parens ( 0 < prio, 
