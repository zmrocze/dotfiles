...
mkCollateralTx pabConf = Constraints.mkTx @Void mempty txc
  where
    ...
    txc = (maybe ) (PaymentPubKeyHash $ pcOwnPubKeyHash pabConf) (collateralValue pabConf)

    mustPayToAddress pkh mskh vl = singleton (MustPayToPubKeyAddress pkh skh Nothing Nothing vl
