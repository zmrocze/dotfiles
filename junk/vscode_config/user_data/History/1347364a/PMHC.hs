...
mkCollateralTx pabConf = Constraints.mkTx @Void mempty txc
  where
    ...
    txc = mustPayToAddress (PaymentPubKeyHash $ pcOwnPubKeyHash pabConf) (pcOwnStakePubKeyHash pabConf) (collateralValue pabConf)

    mustPayToAddress pkh mskh vl = singleton (Constraints.MustPayToPubKeyAddress pkh skh Nothing Nothing vl
