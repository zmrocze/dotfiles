...
import BotPlutusInterface.Effects (prettyLog)
import Data.Text.Prettyprint.Doc (vcat, layoutPretty, defaultLayoutOptions)
  ...
  let txOutRef = TxOutRef "e406b0cf676fc2b1a9edb0617f259ad025c20ea6f0333820aa7cef1bfe7302e5" 0
      ...
      contract = do
        ...
        let collateralInInputs = Set.member collateralTxOutRef $ getCardanoTxInputs tx
            ...
            actualCollaterals = getCardanoTxCollateralInputs tx
            collateralsAreUnexpected = expectedCollaterals /= actualCollaterals
        ...
        when collateralsAreUnexpected $
          throwError @Text
            ( "Unexpected collaterals. Expected: " <> pack (show expectedCollaterals)
                <> ". Actual: "
                <> pack (show actualCollaterals)
            )

  -- run the same contract with two orders of starting utxos
  let (res1, st1) = runContractPure contract (initState utxos1)
      -- (res2, st2) = runContractPure contract (initState utxos2)

  -- if both fail show only one error message
  case res1 of
    Left err -> assertFailure $ Text.unpack err <> "  state1: " <> (
        Render.renderString . layoutPretty defaultLayoutOptions . vcat $ map (\(a, b, c) -> prettyLog a b c) (_logHistory st1)
      )
    Right _ -> pure ()
