...
testTxUsesCollateralCorrectly = do
  ...
  case res1 of
    Left err -> assertFailure $ Text.unpack err <> "  state1: " <> (
        Render.renderString . layoutPretty defaultLayoutOptions . vcat $ map (\(a, b, c) -> prettyLog a b c) (_logHistory @(MockContractState ) ([String]) ([Text]) ([Activity]) w ([(LogContext, LogLevel, Doc ())]) (ContractEnvironment w) ([(TxOutRef, TxOut)]) Tip CollateralUtxo) st1)
      )
    Right _ -> pure ()
