...
saveKeyPair (KeyPair {sKey, vKey}) dir = do
  let 
    skeyDesc, vkeyDesc :: TextEnvelopeDescr
    skeyDesc = "Payment Signing Key"
    vkeyDesc = "Payment Verification Key"

    hash = Text.unpack $ TE.decodeUtf8 $ serialiseToRawBytesHex $ verificationKeyHash vKey

    skeyPath = rmQuotes $ outDir </> sKeyPrefix <> hash <.> "skey"
    vkeyPath = rmQuotes $ outDir </> vKeyPrefix <> hash <.> "vkey"
