...
module Test.Plutip.Internal.Keys (KeyPair, cardanoMainnetAddress, genKeyPair, mainnetAddress, saveKeyPair) where
...
showPkh = TE.pack . CAPI.serialiseToRawBytesHex . CAPI.verificationKeyHash . vKey

saveKeyPair :: FilePath -> KeyPair -> IO (Either (CAPI.FileError ()) ())
saveKeyPair dir KeyPair {sKey, vKey} =
  let
