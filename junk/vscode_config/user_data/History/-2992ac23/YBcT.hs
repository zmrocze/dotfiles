...
signingKeyFilePath pabConf = signingKeyFilePathInDir pabConf.pcSigningKeyFileDir

signingKeyFilePathInDir :: Text -> PubKeyHash -> Text
signingKeyFilePathInDir dir (PubKeyHash pubKeyHash) =
  let h = encodeByteString $ fromBuiltin pubKeyHash
   in dir <> "/signing-key-" <> h <> ".skey"

verificationKeyFilePathInDir :: Text -> PubKeyHash -> Text
verificationKeyFilePathInDir dir (PubKeyHash pubKeyHash) =
  let h = encodeByteString $ fromBuiltin pubKeyHash
   in dir <> "/signing-key-" <> h <> ".vkey"

keyFilePathInDir :: Text -> PubKeyHash -> Text -> Text
keyFilePathInDir dir (PubKeyHash pubKeyHash) ext =
  let h = encodeByteString $ fromBuiltin pubKeyHash
   in dir <> "/signing-key-" <> h <> ext
