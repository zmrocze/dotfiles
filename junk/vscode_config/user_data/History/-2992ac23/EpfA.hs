...
txFilePath pabConf ext txId =         "pcTxFileDir"             <> "/" <> txFileName txId ext
...
data DummyPrivKey
  = FromSKey (SigningKey PaymentKey)
  | FromVKey (VerificationKey PaymentKey)

unDummyPrivateKey :: DummyPrivKey -> Crypto.XPrv
unDummyPrivateKey (FromSKey key) = either (error "dupa") id $ vkeyToDummyPrivKey' $ getVerificationKey key
unDummyPrivateKey (FromVKey key) = either (error "dupa") id $ vkeyToDummyPrivKey' key
