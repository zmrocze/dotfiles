...
import Control.Retry (constantDelay, limitRetries, retrying, RetryPolicyM)
...
data AwaitWalletFundedError
  = AwaitCardanoApiError CardanoApiError 
  | AwaitTimeoutError

instance Show AwaitWalletFundedError where
  show (AwaitCardanoApiError (SomeError e)) = e
  show AwaitTimeoutError = "Awaiting funding transaction timed out."
...
awaitWalletFunded cenv addr = toErrorMsg <$> retrying policy checkResponse action
  where
    ...
    toErrorMsg = \case
      Left e -> Left $ AwaitCardanoApiError e
      Right noUtxos ->
        if noUtxos
          then Left AwaitTimeoutError
          else Right ()
