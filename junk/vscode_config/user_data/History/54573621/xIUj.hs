...
toCardanoMintWitness :: PV1.Redeemer -> Maybe (L.Versioned PV1.MintingPolicy) -> Either ToCardanoError (C.ScriptWitness C.WitCtxMint C.BabbageEra)
toCardanoMintWitness _ Nothing = Left MissingMintingPolicy
toCardanoMintWitness redeemer (Just script) =
  toCardanoScriptWitness C.NoScriptDatumForMint redeemer (fmap getMintingPolicy script)

toCardanoScriptWitness :: PV1.ToData a =>
  C.ScriptDatum witctx
  -> a
  -> L.Versioned PV1.Script
  -> Either ToCardanoError (C.ScriptWitness witctx C.BabbageEra)
toCardanoScriptWitness datum redeemer (L.Versioned lang script) = (case lang of
    L.PlutusV1 ->
      C.PlutusScriptWitness C.PlutusScriptV1InBabbage C.PlutusScriptV1
...
data ToCardanoError
    ...
    | UnsupportedPlutusVersion L.Language
