...
plutusScriptsFromTxBody :: C.TxBody era -> Map P.ScriptHash PV1.Script
...
fromLedgerPlutusScript :: Alonzo.Script a -> Maybe (P.ScriptHash, PV1.Script)
...
-- TODO: Support V2.
toWithdrawals :: Map P.ScriptHash PV1.Script
  -> C.NetworkId
  -> [P.Withdrawal]
...
toCardanoTxInWitness tx
    ...
    = do
      ...
      (PV1.Validator validator) <- maybe (Left MissingInputValidator) pure $ P.lookupValidator (L.txScripts tx) valh
...
toCardanoTxInWitness tx
    ...
    = do
      (PV1.Datum datum) <- maybe (Left MissingDatum) pure $ Map.lookup dh (L.txData tx)
      (PV1.Validator validator) <- maybe (Left MissingInputValidator) pure $ P.lookupValidator (L.txScripts tx) vh
      C.ScriptWitness C.ScriptWitnessForSpending <$>
    ...
    let indexedMps = Map.assocs txMintingScripts
     in C.TxMintValue C.MultiAssetInBabbageEra
        ...
        <*> (C.BuildTxWith . Map.fromList <$> traverse (\(mph, rd) ->
          (,) <$> toCardanoPolicyId mph <*> toCardanoMintWitness rd (P.lookupMintingPolicy (L.txScripts tx) mph)) indexedMps)
