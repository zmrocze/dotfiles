...
toCardanoTxInWitness tx
    ...
    = do
      ...
      (PV1.Validator validator) <- maybe (Left MissingInputValidator) pure $ P.lookupValidator (P.txScripts tx) valh
...
toCardanoTxInWitness tx
    ...
    = do
      (PV1.Datum datum) <- maybe (Left MissingDatum) pure $ Map.lookup dh (P.txData tx)
      (PV1.Validator validator) <- maybe (Left MissingInputValidator) pure $ P.lookupValidator (P.txScripts tx) vh
      C.ScriptWitness C.ScriptWitnessForSpending <$>
