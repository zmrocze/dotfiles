...
import Contract.TextEnvelope (decodeTextEnvelope, plutusScriptV1FromEnvelope, plutusScriptV2FromEnvelope, TextEnvelope)
...
import Effect.Aff (error, throwError, Error)
...
compareApplied ∷ ∀ (m ∷ Type -> Type). Monad m ⇒ MonadError Error m ⇒ (String → m PlutusScript) → String → Tuple (Array PlutusData) String → m Unit
...
v1 ∷ ∀ (m :: Type -> Type). MonadError Error m ⇒ String → m PlutusScript
v1 = lookupAux plutusScriptV1FromEnvelope
v2 ∷ ∀ (m :: Type -> Type). MonadError Error m ⇒ String → m PlutusScript
v2 = lookupAux plutusScriptV2FromEnvelope

lookupAux ∷ ∀ (m ∷ Type -> Type). MonadThrow Error m ⇒ (TextEnvelope → Maybe PlutusScript) → String → m PlutusScript
