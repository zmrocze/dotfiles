...
instance Pretty Tx where
    pretty tx@(Tx _txInputs _txReferenceInputs _txCollateral _txOutputs _txMint _txFee
        ...
        let showNonEmpty empty x = if empty then [] else [x]
            lines' =
                ...
                <> showNonEmpty (Map.empty _txData) $ hang 2 (vsep ("data:": fmap (pretty . snd) (Map.toList _txData)))
                <> showNonEmpty (Map.empty _txScripts) $ hang 2 (vsep ("attached scripts:": fmap pretty (Map.keys _txScripts)))
                <> showNonEmpty (empty _txWithdrawals) $ hang 2 (vsep ("withdrawals:": fmap pretty _txWithdrawals))
