...
import Plutus.V1.Ledger.Tx as Export hiding (TxIn(..), TxInType (..), inRef, inScripts, inType, pubKeyTxIn, pubKeyTxIns, scriptTxIn, scriptTxIns)
import Plutus.V1.Ledger.Tx qualified as V1.Tx hiding (TxIn, TxIn, TxInType (..))
import Data.Set (Set)
...
instance Pretty CardanoTx where
    pretty tx =
        let lines' =
                ...
                , hang 2 (vsep ("redeemers:": fmap (pretty . snd) (Map.toList $ onCardanoTx (txRedeemers) tx) ))
...
updateUtxoCollateral tx unspent = unspent `Map.withoutKeys` (Set.fromList . map txInputRef $ getCardanoTxCollateralInputs tx)
