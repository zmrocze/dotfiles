...
import Test.Plutip.Internal.BotPlutusInterface.Types (BpiWallet (bwTag), EnterpriseInfo (EnterpriseInfo), WithStakeKeysInfo (WithStakeKeysInfo), WalletTag (EnterpriseTag, WithStakeKeysTag), WalletType, WalletTypeError (BadWalletIndex, ExpectedEnterpriseWallet, ExpectedWalletWithStakeKeys), ownAddress, WalletInfo)
...
makeWalletLookups lookups =
  ...
  where
    ...
    lookupTaggedWallet wl (EnterpriseTag k) = case Map.lookup k wl of
      ...
      Just res@(EnterpriseInfo _) -> pure res
      Just (WithStakeKeysInfo _ _) -> throwError $ toError ExpectedEnterpriseWallet
    lookupTaggedWallet wl (WithStakeKeysTag k) = case Map.lookup k wl of
      Nothing -> throwError $ toError BadWalletIndex
      Just (EnterpriseInfo _) -> throwError $ toError ExpectedWalletWithStakeKeys
      Just res@(WithStakeKeysInfo _ _) -> pure res
