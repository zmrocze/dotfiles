...
makeWalletLookups lookups = WalletLookups {
  ...
  lookupAddress = \k -> 
    maybe (throwError $ toE $ toOtherError BadWalletIndex) pure $
      Map.lookup k $ (\(WalletInfo' w) -> ownAddress w) <$> lookups
}
  where
    
    toE1 :: AsContractError e =>  WalletTypeError -> e
    toE1 = toE . toOtherError
    toOtherError = OtherContractError . fromString . show
    toE e = withPrism _ContractError $ \f _ -> f e 
    -- lookupTaggedWallet :: 
    --   forall (k :: Type) (m :: Type -> Type) (t :: WalletType).
    --   (Ord k, MonadError WalletTypeError m) => 
    --   Map k WalletInfo'
    --   -> WalletTag t k
    --   -> m (WalletInfo t)
    lookupTaggedWallet wl (EnterpriseTag k) = case Map.lookup k wl of
      Nothing -> throwError BadWalletIndex
      Just (WalletInfo' res@(EnterpriseInfo _)) -> pure res
      Just (WalletInfo' (WithStakeKeysInfo _ _)) -> throwError ExpectedEnterpriseWallet
    lookupTaggedWallet wl (WithStakeKeysTag k) = case Map.lookup k wl of
      Nothing -> throwError BadWalletIndex
      Just (WalletInfo' (EnterpriseInfo _)) -> throwError ExpectedWalletWithStakeKeys
