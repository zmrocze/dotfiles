...
import Ledger (Ada, CardanoTx (EmulatorTx), CurrencySymbol, Datum, Interval, OnChainTx (Valid),
               ...
               addMintingPolicy, pubKeyTxOut, scriptHash, toPublicKey, txData, txScripts, validatorHash, getValidator)
    ...
    if fee' < totalValAda
        then do
            ...
            let tx = mempty
                        ...
                        , txScripts = Map.fromList (map (\s -> (scriptHash (fmap getValidator s), s)) scripts)
                        }
