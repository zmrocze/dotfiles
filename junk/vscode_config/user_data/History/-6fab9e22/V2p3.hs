...
import Ledger (Ada, CardanoTx (EmulatorTx), CurrencySymbol, Datum, Interval, OnChainTx (Valid),
               ...
               Tx (txFee, txInputs, txMint, txMintingScripts, txOutputs, txValidRange), TxIn,
               ...
               addMintingPolicy, addSignature', pubKeyTxIn, pubKeyTxOut, toPublicKey, txData, txId,
               txScripts)
...
genValidTransactionSpending' g ins totalVal = do
    ...
    where
        ...
        txInToTxInput (TxInputWitnessed outref txInType) = case txInType of
            ...
            Ledger.ConsumeScriptAddress pv vl rd dt -> (TxInput outref (Ledger.TxConsumeScriptAddress pv rd (Script.ValidatorHash vl) (datumHash dt)), Just (vl, dt))
