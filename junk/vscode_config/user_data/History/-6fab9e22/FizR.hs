...
import Ledger (Ada, CardanoTx (EmulatorTx), CurrencySymbol, Datum, Interval, OnChainTx (Valid),
               ...
               ValidationCtx (ValidationCtx), Value, _runValidation, addCardanoTxSignature,
               addMintingPolicy, getValidator, pubKeyTxOut, scriptHash, toPublicKey, txData, txScripts, validatorHash, Versioned, Language(PlutusV1), Validator)
...
import Plutus.Script.Utils.Scripts (datumHash, Versioned (Versioned))
    ...
    if fee' < totalValAda
        then do
            ...
            let tx = mempty
                    ...
                    & addMintingPolicy (Versioned PlutusV1 ScriptGen.alwaysSucceedPolicy) Script.unitRedeemer
                    & EmulatorTx
