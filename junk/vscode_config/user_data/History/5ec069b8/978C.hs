...
import Ledger.Tx.Internal (Tx(..), updateUtxoCollateral, fillTxInputWitnesses, TxInput, strip, spentOutputs, txInputRef)
...
import Plutus.V1.Ledger.Tx (TxOut (..), TxIn(..), TxOutRef(..))
import Plutus.V1.Ledger.Tx qualified as Export
...
getCardanoTxSpentOutputs = map txInputRef . getCardanoTxInputs
