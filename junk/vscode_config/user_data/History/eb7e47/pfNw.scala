...
class MySuite extends munit.FunSuite {
  ...
  val phidebug : GFFormula[Int] = 
    ands(List(
      And(Forall(List(1), atom("P", List(1)), Not(Atom(atom("N", List(1))))), 
          Forall(List(2), atom("N", List(2)), Not(Atom(atom("P", List(2)))))),
      Forall(List(2,3), atom("S", List(2, 3)), 
        And(Or( aatom("P", List(2)), 
              aatom("P", List(3)) ),
            Or( aatom("N", List(2)), 
              aatom("N", List(3)) ))),
      Exist(List(6), atom("P", List(6)), aatom("P", List(6))),
      Forall(List(4), atom("P", List(4)), 
        Exist(List(5), atom("S", List(4,5)), aatom("S", List(4,5))))
      ))
  test("Pairs formula"){ 
    println(parity.pretty())
    println(phidebug.pretty())
