...
import Test.Plutip.Internal.BotPlutusInterface.Types (BpiError (BotInterfaceDirMissing, SignKeySaveError), BpiWallet (BpiWallet), TestWallet (twInitDistribiution), payKeys, stakeKeys, TestWallet', WalletTag (EnterpriseTag, WithStakeKeysTag))
...
createWallet tag = do 
  ...
  skp <- case tag of 
    EnterpriseTag k -> pure Nothing
    WithStakeKeysTag k -> fmap Just genStakeKeyPair
