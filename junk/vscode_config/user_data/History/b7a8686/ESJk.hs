...
import Test.Plutip.Internal.BotPlutusInterface.Types (BpiError (BotInterfaceDirMissing, SignKeySaveError), BpiWallet (BpiWallet), TestWallet (twInitDistribiution, testWalletTag), payKeys, stakeKeys, TestWallet' (TestWallet'), WalletTag (EnterpriseTag, WithStakeKeysTag))
...
eitherAddSomeWalletDir (TestWallet' funds) wallDir = do
  bpiWallet <- createWallet (testWalletTag funds)
