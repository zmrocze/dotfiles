...
import Ledger (PaymentPubKeyHash (PaymentPubKeyHash), PubKeyHash (PubKeyHash), StakePubKeyHash (StakePubKeyHash), Value)
...
import Test.Plutip.Internal.BotPlutusInterface.Types (BpiError (BotInterfaceDirMissing, SignKeySaveError), BpiWallet (BpiWallet), TestWallet, payKeys, stakeKeys)
    ...
    sendFunds wallet = do
      ...
      liftIO $
        sendFaucetFundsTo
          ...
          (nodeSocket cEnv)
          (supportDir cEnv)
          [(fundAddress, toAmt v) | v <- twInitDistribution funds]
