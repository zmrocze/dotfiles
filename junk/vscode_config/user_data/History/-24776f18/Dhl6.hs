...
import Ledger.Tx (CardanoTx, TxOut, TxOutRef (..),
                  collateralInputs, fillTxInputWitnesses, getCardanoTxId, getCardanoTxOutputs, inputs, spentOutputs,
                  txOutValue, unspentOutputsTx, updateUtxo, updateUtxoCollateral, validValuesTx, getCardanoTxInputs, getCardanoTxUnspentOutputsTx, getCardanoTxSpentOutputs, getCardanoTxCollateralInputs)
...
import Plutus.V1.Ledger.Tx (TxIn (txInRef), txOutPubKey, txOutDatum)
...
unspentOutputs = foldr (eitherTx updateUtxoCollateral_ updateUtxo_) Map.empty . join
    where 
        -- | Update a map of unspent transaction outputs and signatures
        --   for a failed transaction using its collateral inputs.
        updateUtxoCollateral_ :: CardanoTx -> Map TxOutRef TxOut -> Map TxOutRef TxOut
        updateUtxoCollateral_ tx unspent = unspent `Map.withoutKeys` (Set.fromList $ map txInRef . getCardanoTxCollateralInputs $ tx)

        updateUtxo_ :: CardanoTx -> Map TxOutRef TxOut -> Map TxOutRef TxOut
        updateUtxo_ tx unspent = (unspent `Map.withoutKeys` (Set.fromList $ getCardanoTxSpentOutputs tx)) `Map.union` getCardanoTxUnspentOutputsTx tx
