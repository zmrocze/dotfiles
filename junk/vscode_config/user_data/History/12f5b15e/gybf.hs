...
import Prelude
import BotPlutusInterface.Collateral (removeCollateral)
...
chainIndexUtxoQuery contractEnv query = do
  ...
  let removeCollateralFromResponse (UtxosResponse tip page) = UtxosResponse tip (removeCollateral collateralUtxo page)
  removeCollateralFromResponse
    <$> chainIndexQueryMany
...
chainIndexTxoQuery contractEnv query = do
  collateralUtxo <- readCollateralUtxo contractEnv
  let removeCollateralFromResponse (TxosResponse page) = TxosResponse (removeCollateral collateralUtxo page)
  removeCollateralFromResponse
    <$> chainIndexQueryMany
      contractEnv.cePABConfig
      query
