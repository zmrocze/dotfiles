...
import BotPlutusInterface.Types (PABConfig, cePABConfig, ContractEnvironment, readCollateralUtxo)
...
import Plutus.ChainIndex.Api (
  ...
  UtxoWithCurrencyRequest (UtxoWithCurrencyRequest), UtxosResponse (UtxosResponse), TxosResponse (TxosResponse)
 )
...
handleChainIndexReq contractEnv = let pabConf = contractEnv.cePABConfig in \case
  ...
  TxoSetAtAddress page credential ->
    TxoSetAtResponse <$> chainIndexTxoQuery contractEnv
