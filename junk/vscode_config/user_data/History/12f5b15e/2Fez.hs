...
chainIndexUtxoQuery :: forall (w :: Type). ContractEnvironment w -> ClientM UtxosResponse -> IO _
chainIndexUtxoQuery contractEnv a = do
    collateralUtxo <- readCollateralUtxo contractEnv
    removeUtxo collateralUtxo
      <$> chainIndexQueryMany
...
removeUtxo :: Maybe TxOutRef -> UtxosResponse -> UtxosResponse
removeUtxo = \case 
  Nothing -> id
  (Just txOutRef) -> \(UtxosResponse tip page) ->
    UtxosResponse tip $ page { pageItems = filter (/= txOutRef) (pageItems page) }
