...
sealed trait StructedSubFormula[X] {
    def toGFormula(): GFFormula[X] = this match
        ...
        case StructAnd(left, right) => And(left, right)
        case StructOr(left, right) => Or(left, right)
        case StructExist(variables, guard, sub) => Exist(variables, guard, sub)
