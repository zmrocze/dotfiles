...
import Control.Monad.Freer (Eff, Member, interpret, reinterpret, runM, subsume, type (~>), send)
...
data LogPABRequests a where
  LogResponse :: PABResp -> LogPABRequests ()
  LogRequest  :: PABReq  -> LogPABRequests ()

-- toLogRequests :: Resumable PABResp PABReq ~> LogPABRequests
-- toLogRequests (RRequest o) = 
appendRequestLogs ::  forall (w :: Type) (effs :: [Type -> Type]).
  (Member (PABEffect w) effs, Member LogPABRequests effs) =>
  (PABReq -> Eff effs PABResp) -> (PABReq -> Eff effs PABResp)
appendRequestLogs respond req = do
  send $ LogRequest req
  resp <- respond req
  send $ LogResponse resp
  return resp
