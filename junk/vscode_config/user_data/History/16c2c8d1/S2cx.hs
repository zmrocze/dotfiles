...
--  - last one - `PaymentPubKeyHash` of wallet with 102 Ada
--
--
...
--    >   "Send some Ada"
--    >   (initAda (PkhTag "pkh0") 100 <> initAda (PkhTag "pkh1") 101 <> initAda (PkhTag "pkh2") 102)
...
--    >         withContract $ \wl -> 
--    >           lookupWallet wl (PkhTag 0) >>= \(PkhWallet pkh1) -> payTo pkh1 10_000_000
--    >       withContractAs 1 $ -- run the contract which result will be checked
--    >         \wl -> lookupWallet wl (PkhTag 0) >>= \(PkhWallet pkh0) -> payTo pkh0 10_000_000
--    >   )
--    >   [shouldSucceed]
--
