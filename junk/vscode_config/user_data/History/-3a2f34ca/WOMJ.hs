...
module Plutus.V1.Ledger.Scripts(
    ...
    MintingPolicyHash (..),
    StakeValidatorHash (..),
    scriptHash, validatorHash, datumHash, redeemerHash, mintingPolicyHash, stakeValidatorHash) where
...
{-# INLINE checkMaxLengthConstructor #-}
-- | Check if ByteString has length <= len and apply constructor f. 
checkMaxLengthConstructor :: Haskell.Int -> (BuiltinByteString -> a) -> BS.ByteString -> Maybe a
checkMaxLengthConstructor len f bs =
...
-- | Make ScriptHash, checks if length <= 28.
scriptHash :: BS.ByteString -> Maybe ScriptHash
scriptHash = checkMaxLengthConstructor 28 UnsafeScriptHash
...
-- | Make ValidatorHash, checks if length <= 28.
validatorHash :: BS.ByteString -> Maybe ValidatorHash
validatorHash = checkMaxLengthConstructor 28 UnsafeValidatorHash
...
-- | Make DatumHash, checks if length <= 32.
datumHash :: BS.ByteString -> Maybe DatumHash
datumHash = checkMaxLengthConstructor 32 UnsafeDatumHash
...
redeemerHash = checkMaxLengthConstructor 32 UnsafeRedeemerHash
...
mintingPolicyHash = checkMaxLengthConstructor 28 UnsafeMintingPolicyHash
...
stakeValidatorHash = checkMaxLengthConstructor 28 UnsafeStakeValidatorHash

-- correctLength :: Int -> ByteString -> Maybe ByteString
