...
import Effect.Aff (Aff, Canceler(Canceler), launchAff_, makeAff, try)
        ...
        withBrowser opts.noHeadless rt (Just extensionId) \browser -> do
          withE2ETest (wrap url) browser \re@{ page } -> do
            ...
            subscribeToBrowserEvents (Just $ Seconds 10.0) page
              case _ of
                ConfirmAccess -> launchAff_ $ try someWallet.confirmAccess
                Sign -> launchAff_ $ try someWallet.sign
                Success -> pure unit
