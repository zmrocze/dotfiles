...
import Effect.Aff (Aff, Canceler(Canceler), fiberCanceler, forkAff, launchAff, launchAff_, makeAff, nonCanceler, try)
import Effect.Aff.Class (liftAff)
import Effect.Class (liftEffect)
import Effect.Exception (Error, error, throw, throwException)
      ...
      { url, wallet: WalletExtension wallet } -> do
        ...
        withBrowser opts.noHeadless rt (Just extensionId) \browser -> do
          withE2ETest (wrap url) browser \re@{ page } -> do
            ...
            makeAff $ \k -> do
              (fmap fiberCanceler $ launchAff $ subscribeToBrowserEvents (Just $ Seconds 10.0) page
                case _ of
                  ConfirmAccess -> launchAff_ $ do
                    res <- try $ someWallet.confirmAccess
                    if isLeft res then 
                      liftEffect $ k res
                    else
                      (pure unit :: Aff Unit) 
