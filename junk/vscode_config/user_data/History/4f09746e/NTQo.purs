...
import Effect.Aff (Aff, Canceler(Canceler), fiberCanceler, launchAff_, makeAff)
      ...
      { url, wallet: WalletExtension wallet } -> do
        ...
        withBrowser opts.noHeadless rt (Just extensionId) \browser -> do
          withE2ETest (wrap url) browser \re@{ page } -> do
            ...
            makeAff $ \k -> fiberCanceler <$> launchAff $ subscribeToBrowserEvents (Just $ Seconds 10.0) page
              case _ of
