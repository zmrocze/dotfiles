...
import Effect.Aff (Aff, Canceler(Canceler), forkAff, launchAff_, makeAff, try)
      ...
      { url, wallet: WalletExtension wallet } -> do
        ...
        withBrowser opts.noHeadless rt (Just extensionId) \browser -> do
          withE2ETest (wrap url) browser \re@{ page } -> do
            ...
            makeAff $ \k -> do
              resRef <- Ref.new Nothing
              liftEffect (subscribeToBrowserEvents (Just $ Seconds 10.0) page
                case _ of
