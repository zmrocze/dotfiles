...
import BlockType
    ( blockBlockHeight,
      ...
      BlockReference, Output (Output), Transaction (outputs, inputs), Input (Input), TXID )
...
newfixed2SpentTXOSet :: [Block] -> Set (TXID, Integer)
newfixed2SpentTXOSet newfixed = Set.fromList 
    [(txid, vout) | block <- newfixed, 
              tx <- transactions block,
              Input _ _ txid vout   <- inputs tx]
