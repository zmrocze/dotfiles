...
def mguResolvedClause[X](AA : SkolemLiteral[X], BB : SkolemLiteral[X], c1 : UqClause[X], c2 : UqClause[X])(implicit ord: Ordering[X]): Option[UqClause[Int]] = {
    ...
    s match
        ...
        case Some(x) => {
            ...
            def rename[Y](lr : X => Either[X,X])( cs : Set[SkolemLiteral[Y]]) = {
                cs.map( l => l.fmaped[Either[X,X]](lr).fmaped(normalization.apply) )
            }
            UqClause( rename(Left(_))(c1.psis excl AA) union rename(Right(_))(c2.psis excl BB) )
            // UqClause((c1.psis excl AA) union (c2.psis excl BB).fmaped(Left(_)))
        }
            // x. c1. fmaped
