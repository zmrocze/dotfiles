...
def mguResolvedClause[X](A : AtomicFormula[X], B : AtomicFormula[X], c1 : UqClause[X], c2 : UqClause[X])(implicit ord: Ordering[X]): Option[Map[X, Term[X]]] = {
    ...
    val s = mgu[Either[X, X]](A1, B1)(lexOrd)
    s match
        case None => None 
        case Some(x) => {
