...
import Ctl.Internal.Plutip.Spawn (ManagedProcess, NewOutputAction(Success, NoOp), cleanupTmpDir, spawn, stop, waitForStop)
...
import Ctl.Internal.QueryM.Ogmios (EpochLength(..), SlotLength(..))
...
import Node.ChildProcess (StdIOBehaviour(..), defaultSpawnOptions, inherit)
...
startPlutipServer cfg = do
  spawn "plutip-server" [ "-p", UInt.toString cfg.port ]
    (defaultSpawnOptions  { stdio = [Just Pipe, Just Pipe, shareStdErr] })
    Nothing
  where 
    shareStdErr = match inherit with 
