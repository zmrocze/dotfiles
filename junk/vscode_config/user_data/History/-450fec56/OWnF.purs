...
newtype TransactionOutputV1 = TxOut {
    txOutAddress   :: Address
  , txOutValue     :: Value
  , txOutDatumHash :: Maybe DataHash
}

derive instance Eq TransactionOutputV1
derive instance Newtype TransactionOutputV1 _
derive instance Generic TransactionOutputV1 _

instance Show TransactionOutputV1 where 
  show = genericShow

instance ToData TransactionOutputV1 where 
  toData TxOut = 
...
derive newtype instance ToData TxInInfoV1
...
instance ToData DCert where 
  toData x = case x of
    DCertDelegRegKey cred -> Constr (fromInt 0) [toData cred]
    DCertDelegDeRegKey cred -> Constr (fromInt 1) [toData cred]
    DCertDelegDelegate cred pkh -> Constr (fromInt 2) [toData cred, toData pkh]
    DCertPoolRegister pkh1 pkh2 -> Constr (fromInt 3) [toData pkh1, toData pkh2]
    DCertPoolRetire pkh n -> Constr (fromInt 4) [toData pkh, toData n]
    DCertGenesis -> Constr (fromInt 5) []
    DCertMir -> Constr (fromInt 6) []
...
instance ToData TxInfoV1 where 
  toData (TxInfoV1 {
  ...
  , txInfoSignatories: txInfoSignatories
  , txInfoData: txInfoData
  , txInfoId: txInfoId
  }) = Constr (fromInt 0) [
      toData txInfoInputs
    , toData txInfoOutputs
    , toData txInfoFee
    , toData txInfoMint
    , toData txInfoDCert
    , toData txInfoWdrl
    , toData txInfoValidRange
    , toData txInfoSignatories
    , toData txInfoData
    , toData txInfoId
  ]