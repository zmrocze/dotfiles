...
toCardanoTxBodyContent sigs protocolParams networkId
  tx@P.Tx{txInputs, txCollateral, txOutputs, txMint, txFee, txValidRange, txMintingScripts, txWithdrawals, txCertificates, txSignatures, txScripts, txData} = do
...
toWithdrawals :: Map P.ScriptHash P.Script
  -> C.NetworkId
  -> [P.Withdrawal]
...
toWithdrawals txScripts networkId = \case
  ...
  where
    ...
    toStakeWitness withdrawalRedeemer cred = case cred of
      ...
      P.ScriptCredential _vh -> case (,) <$> withdrawalRedeemer <*> P.lookupValidator txScripts _vh of
...
toCardanoTxInBuild :: P.Tx -> TxInput -> Either ToCardanoError (C.TxIn, C.BuildTxWith C.BuildTx (C.Witness C.WitCtxTxIn C.AlonzoEra))
toCardanoTxInBuild tx (TxInput txInRef txInType) = (,) <$> toCardanoTxIn txInRef <*> (C.BuildTxWith <$> toCardanoTxInWitness tx txInType)
...
toCardanoTxInsCollateral :: [TxInput] -> Either ToCardanoError (C.TxInsCollateral C.AlonzoEra)
toCardanoTxInsCollateral = fmap (C.TxInsCollateral C.CollateralInAlonzoEra) . traverse (toCardanoTxIn . P.txInputRef)
...
toCardanoTxInWitness :: P.Tx -> P.TxInputType -> Either ToCardanoError (C.Witness C.WitCtxTxIn C.AlonzoEra)
...
toCardanoTxInWitness tx
    (P.TxConsumeScriptAddress
        ...
        datumHash)
    = do
      (P.Datum datum) <- maybe (Left MissingDatum) pure $ Map.lookup datumHash (P.txData tx)
      (P.Validator validator) <- maybe (Left MissingInputValidator) pure $ P.lookupValidator (txScripts tx) validatorHash
...
toCardanoMintValue :: P.Tx -> Either ToCardanoError (C.TxMintValue C.BuildTx C.AlonzoEra)
    ...
    let indexedMps = Map.assocs txMintingScripts
     in C.TxMintValue C.MultiAssetInAlonzoEra
        ...
        <*> (C.BuildTxWith . Map.fromList <$> traverse (\(mph, rd) ->
          (,) <$> toCardanoPolicyId mph <*> toCardanoMintWitness rd (L.lookupMintingPolicy (P.txScripts tx) mph)) indexedMps)
