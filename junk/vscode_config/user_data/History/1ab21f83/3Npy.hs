...
toCardanoTxInWitness :: P.Tx -> L.TxInputType -> Either ToCardanoError (C.Witness C.WitCtxTxIn C.AlonzoEra)
...
toCardanoTxInWitness tx
    ...
    = do
      ...
      (P.Validator validator) <- maybe (Left MissingInputValidator) pure $ L.lookupValidator (txScripts tx) validatorHash
    ...
    let indexedMps = Map.assocs txMintingScripts
     in C.TxMintValue C.MultiAssetInAlonzoEra
        ...
        <*> (C.BuildTxWith . Map.fromList <$> traverse (\(mph, rd) ->
          (,) <$> toCardanoPolicyId mph <*> toCardanoMintWitness rd (L.lookupMintingPolicy (txScripts tx) mph)) indexedMps)
