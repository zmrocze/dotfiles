...
toCardanoTxBodyContent sigs protocolParams networkId
  tx@Tx{txInputs, txCollateral, txOutputs,  txFee, txValidRange, txMintingScripts, txWithdrawals, txCertificates, txSignatures, txScripts, txData} = do
...
toCardanoTxInBuild :: Tx -> L.TxInput -> Either ToCardanoError (C.TxIn, C.BuildTxWith C.BuildTx (C.Witness C.WitCtxTxIn C.AlonzoEra))
...
toCardanoTxInWitness :: Tx -> L.TxInputType -> Either ToCardanoError (C.Witness C.WitCtxTxIn C.AlonzoEra)
...
toCardanoTxInWitness tx
    (L.TxConsumeScriptAddress
        ...
        datumHash)
    = do
      (P.Datum datum) <- maybe (Left MissingDatum) pure $ Map.lookup datumHash (txData tx)
    ...
    let indexedMps = Map.assocs txMintingScripts
     in C.TxMintValue C.MultiAssetInAlonzoEra
        ...
        <*> (C.BuildTxWith . Map.fromList <$> traverse (\(mph, rd) ->
          (,) <$> toCardanoPolicyId mph <*> toCardanoMintWitness rd (L.lookupMintingPolicy txScripts mph)) indexedMps)
