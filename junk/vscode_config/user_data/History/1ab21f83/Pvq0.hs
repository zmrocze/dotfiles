...
toCardanoTxInBuild :: P.Tx -> P.TxInput -> Either ToCardanoError (C.TxIn, C.BuildTxWith C.BuildTx (C.Witness C.WitCtxTxIn C.AlonzoEra))
...
toCardanoTxInsCollateral :: [P.TxInput] -> Either ToCardanoError (C.TxInsCollateral C.AlonzoEra)
...
toCardanoTxInWitness tx
    ...
    = do
      ...
      (P.Validator validator) <- maybe (Left MissingInputValidator) pure $ P.lookupValidator (P.txScripts tx) validatorHash
