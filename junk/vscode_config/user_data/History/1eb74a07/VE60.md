...
Though the validation logic checks precisely for a single `Upgrade Token` UTXO to get burnt. This allows for a transaction spending two `Upgrade Tokens` like above, burning one of them as demanded by the validation and sending remaining one to a wallet address of an attacker.
