...
import Test.Plutip.Internal.BotPlutusInterface.Types (TestWallets (unTestWallets, TestWallets), SomeBpiWallet (SomeBpiWallet), BpiWallet (bwTag), TestWallet (twTag), getTag, TestWallet' (TestWallet'))
...
withConfiguredCluster conf name testCases =
  withResource (startCluster conf setup) (stopCluster . fst) $
    \getResource ->
      testGroup name $
        imap
          (\idx (ClusterTest (tws, toTestGroup)) -> toTestGroup $ second (substituteTags . (!! idx)) . snd <$> getResource)
          testCases
  where
    ...
    setup = do
      ...
      wallets <-
        traverse
          (traverse (fmap SomeBpiWallet . addSomeWallet) . unTestWallets . (\(ClusterTest (tws, _)) -> tws))
          testCases
    ...
    substituteTags (TestWallets tws) walls = NonEmpty.zipWith (\(TestWallet' tw) (SomeBpiWallet bw) -> bw {bwTag = getTag (twTag tw)}) tws walls
