...
parse_lvalue:
  ...
  | arr = parse_node(parse_expression_legal_in_lvalue) ; BRACE_SQUARE_LEFT ; ind= parse_node(parse_expression) ; BRACE_SQUARE_RIGHT 
    { LVALUE_Index { expr = arr ; index = ind } } %prec ARR_INDEX
  | e = parse_node(parse_expression_legal_in_lvalue) ; DOT ; id = identifier
    { LVALUE_Field { expr = e ; field = id } } %prec FIELD_SELECT
...
%inline parse_expression_legal_in_lvalue:
  | v = parse_lvalue
    { match v with
...
%inline parse_expression_remaining:
  ...
  | NOT ; e = parse_node(parse_expression)
    { EXPR_Unop { op = UNOP_Not ; expr = e} } 
      // %prec NOT
  | MINUS ; e = parse_node(parse_expression)
    { EXPR_Unop { op = UNOP_Neg ; expr = e} } 
      // %prec MINUS_UNOP
...
parse_statement_not_empty_return:
  ...
  | IF ; bl = parse_node(parse_expression) ; s1 = parse_node(parse_statement) ; opt_s2 = option(preceded(ELSE, parse_node(parse_statement)))
    { STMT_If { cond = bl ; then_branch = s1 ; else_branch = opt_s2 } } %prec IF_ELSE
