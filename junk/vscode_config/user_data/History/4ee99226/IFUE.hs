...
import Test.Plutip.Contract (
  ...
  initLovelace,
  withContract,
  withContractAs, assertExecutionWith
 )
...
import Test.Plutip.Options (TraceOption(TracingButOnlyContext, BudgetCounting))
...
test =
  withConfiguredCluster
    ...
    , assertExecutionWith
        [TracingButOnlyContext ContractLog Debug] -- show 
        "Contract 3"
        (initAda [100])
        (withContract $ const $ Contract.logInfo @Text "Seconds before disaster." >> getUtxosThrowsEx)
        [ shouldFail
        , Predicate.not shouldSucceed
        ]
    , assertExecution
    ...
    , -- tests with assertions on execution budget
      assertExecutionWith 
        [BudgetCounting]  -- this influences displaying the budgets only and is not necessary for budget assertions
