...
{-# INLINEABLE validityRangeFiniteLowerBound #-}
validityRangeFiniteLowerBound :: V2.TxInfo -> Maybe Ledger.POSIXTime
validityRangeFiniteLowerBound info = case Ledger.ivFrom $ V2.txInfoValidRange info of
  Ledger.LowerBound (Ledger.Finite lowerTime) closure ->
    Just $
      if closure
        then lowerTime
        else lowerTime + 1
...
{-# INLINEABLE checkOwnOutputNoStakingAdaGeq #-}
checkOwnOutputNoStakingAdaGeq :: (PlutusTx.ToData a) => V2.ScriptContext -> a -> Ledger.Value -> Bool
checkOwnOutputNoStakingAdaGeq ctx datum val =
  let d = serializeDatum datum
      outputs = getContinuingOutputsNoStaking ctx
   in any (checkOutputAdaGeq d val) outputs
...
{-# INLINEABLE checkOutputValAdaGeq #-}
checkOutputValAdaGeq :: Ledger.Value -> V2.TxOut -> Bool
checkOutputValAdaGeq val TxOut {txOutValue} =
  case Value.flattenValue (txOutValue - val) of
    [] -> True
    [(symbol, token, amount)] ->
      symbol == Ada.adaSymbol
...
{-# INLINEABLE checkOutputToAddressAdaGeq #-}
checkOutputToAddressAdaGeq :: V2.TxInfo -> V2.Address -> Ledger.Value -> Bool
checkOutputToAddressAdaGeq info address val =
  any
    (checkOutputValAdaGeq val)
    ( filter
        (\V2.TxOut {txOutAddress} -> txOutAddress == address)
        (V2.txInfoOutputs info)
    )
