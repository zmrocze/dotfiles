...
main = do
  ...
  let yy = xx v1 v1scriptPaths a
  _
  where

    xx lang paths f = map (uncurry (f lang)) $
      (,) <$> map (lang,) paths  <*> paramss

    a lang scrpath (args, pname) = do
      scr <- loadScript lang scrpath
      let res = applyArguments scr args
      res <- maybe (die "fuck") id $ deserialiseFromRawBytes (proxyToAsType lang) $ toStrict $ serialise res
