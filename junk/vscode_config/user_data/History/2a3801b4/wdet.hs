...
main = do 
  print $ renderString $ layoutPretty defaultLayoutOptions $ pretty contextData
...
mainMut = do
  ...
  where
    ...
    a lang scrpath (args, pname) = do
      ...
      res <- maybe (die "fuck") pure $ deserialiseFromRawBytes (proxyToAsType lang) $ toStrict $ serialise res1
      let path = ("plutus-scripts/applied/" <> scrpath <> "-" <> pname <> ".plutus")
      writeFile path "" 
      r <- writeFileTextEnvelope path Nothing res
