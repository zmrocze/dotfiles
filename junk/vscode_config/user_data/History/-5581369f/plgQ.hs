...
collectLog logs msgLogCtx msgLogLvl msg = atomically $ modifyTVar' logs (appendLog logCtx msgLogLvl msg)
  where
    appendLog :: LogContext -> LogLevel -> PP.Doc () -> LogsList -> LogsList
    appendLog logCtx logLvl str (LogsList ls) = LogsList $ (logCtx, logLvl, str) : ls
