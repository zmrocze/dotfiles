  ...
  interpretM
    ( \case
        ...
        PrintLog logCtx logLevel msg -> do
          printLog' contractEnv.cePABConfig.pcLogLevel logCtx logLevel msg
          when contractEnv.cePABConfig.pcCollectLogs $
            collectLog contractEnv.ceContractLogs logCtx logLevel msg
  ...
  where
    target =
      Render.renderString . layoutPretty defaultLayoutOptions $
        pretty $ LogLine msgCtx msgLogLvl msg
...
collectLog logs logCtx logLvl msg = atomically $ modifyTVar' logs appendLog
  where
    appendLog (LogsList ls) = LogsList $ LogLine logCtx logLvl msg : ls
