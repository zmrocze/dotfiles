...
validateStabilityPool params StabilityPoolDatum {..} CreateAccount {..} ctx =
  validateCreateAccount
    ...
    caAmount
    ctx
validateStabilityPool params StabilityPoolDatum {..} AdjustAccount {..} ctx =
...
validateRecordEpochToScaleToSum
  ...
  ctx =
    traceIfFalse "Invalid input being validated" (ownRef == poolRef)
      ...
      && traceIfFalse "snapshot map is empty" (not (null snapshotMapItems))
    where
      info :: V2.TxInfo
...
validateClose
  ...
  ctx =
    ...
    where
      ...
      poolSnapshot :: StabilityPoolSnapshot
      poolSnapshot =
        StabilityPoolSnapshot p (d |-| fund) s snapshotEpoch snapshotScale
  ...
  ctx =
    traceIfFalse "Invalid input being validated" (ownRef == poolRef)
      && traceIfFalse
        ...
        (Contexts.txSignedBy info (Ledger.unPaymentPubKeyHash pk))
      && traceIfFalse "The asset does not match" (asset == aAsset)
      && traceIfFalse "User cannot have negative balance" (changedAmount >= 0)
        ...
        ( Helpers.checkOwnOutput
            ...
            $ poolValue
              ...
              <> V2.singleton
                assetSymbol
                asset
                (changedAmount - fromSPInteger fund)
        )
      && traceIfFalse
    ...
    where
      ...
      StabilityPoolSnapshot p d s snapshotEpoch scale = snapshot
      ...
      poolSnapshot =
        StabilityPoolSnapshot
          p
          (d |-| fund |+| toSPInteger changedAmount)
          s
          snapshotEpoch
          scale
...
validateCreateAccount
  ...
  ctx =
    traceIfFalse "Invalid input being validated" (ownRef == poolRef)
      ...
      && traceIfFalse
        ...
        ( Helpers.checkOwnOutput
            ctx
            (StabilityPoolDatum asset poolSnapshot epochToScaleToSum)
            (poolValue <> V2.singleton assetSymbol asset amt)
        )
      && traceIfFalse
        "Account output does not match"
        ( Helpers.checkOwnOutputAdaGeq
    ...
    where
      ...
      poolSnapshot :: StabilityPoolSnapshot
      poolSnapshot =
        StabilityPoolSnapshot p (d |+| toSPInteger amt) s snapshotEpoch scale

      accountSnapshot :: StabilityPoolSnapshot
      accountSnapshot =
        StabilityPoolSnapshot p (toSPInteger amt) s snapshotEpoch scale
