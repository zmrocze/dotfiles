use minicbor::{data::Tag, Decode, Encode, bytes::ByteVec};
...
pub struct Bytes(minicbor::bytes::ByteVec);
...
impl<'b, C> minicbor::decode::Decode<'b, C> for Bytes
where
{
    fn decode(d: &mut minicbor::Decoder<'b>, ctx: &mut C) -> Result<Self, minicbor::decode::Error> {
        let mut res = Vec::new();
        let bsi = d.bytes_iter()?;
        let first_chunk = bsi.next().transpose()?;
        match first_chunk {
            None => return Err(minicbor::decode::Error::end_of_input()),
            Some(ch) => res.extend_from_slice(ch)
        }
        for e_chunk in bsi {
            let chunk = e_chunk?;
