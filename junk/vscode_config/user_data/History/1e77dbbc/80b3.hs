...
import Ledger (Address (addressCredential), CardanoTx, ChainIndexTxOut,
               ...
               Tx (txFee, txMint), TxIn (TxIn, txInRef), TxOutRef, UtxoIndex (..), ValidatorHash, Value, TxInput (txInputRef, TxInput))
...
addCollateral mp vl tx = do
    ...
    let addTxCollateral =
            let ins = Set.fromList ((`Tx.TxInput` Tx.TxConsumePublicKeyAddress) . fst <$> spend)
            -- Is here uniqueness needed?
