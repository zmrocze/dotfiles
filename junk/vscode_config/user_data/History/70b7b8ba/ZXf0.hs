        ...
        ( \filename ->
            let fullPath = Text.unpack pabConf.pcSigningKeyFileDir </> filename
             in case takeExtension filename of
                  ".vkey" -> when (isPrefixOf "signing-key" filename) $ Just <$> readVerificationKey @w fullPath
                  ".skey" -> when (isPrefixOf "verification-key" filename) $ Just <$> readSigningKey @w fullPath
                  _ -> pure Nothing
        )
        files
