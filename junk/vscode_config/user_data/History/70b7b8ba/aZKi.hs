      ...
      <$> mapM
        ( \filename ->
            let fullPath = Text.unpack pabConf.pcSigningKeyFileDir </> filename
             in case (( || isPrefixOf "verification-key" filename), takeExtension filename) of
                  (True, ".vkey") -> when (isPrefixOf "signing-key" filename) Just <$> readVerificationKey @w fullPath
                  (True, ".skey") -> Just <$> readSigningKey @w fullPath
                  _ -> pure Nothing)
        )
        files
