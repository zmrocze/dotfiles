        ...
        ( \filename ->
            let fullPath = Text.unpack pabConf.pcSigningKeyFileDir </> filename
             in case takeExtension filename of
                  ".vkey" -> if "signing-key" `isPrefixOf` filename then Just <$> readVerificationKey @w fullPath else pure Nothing
                  ".skey" -> if "verification-key" `isPrefixOf` filename then Just <$> readSigningKey @w fullPath else pure Nothing
                  _ -> pure Nothing
        )
        files
