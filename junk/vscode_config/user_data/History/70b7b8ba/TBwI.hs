        ...
        ( \filename ->
            let fullPath = Text.unpack         "pcSigningKeyFileDir"                     </> filename
             in case takeExtension filename of
                  ".vkey" -> if ("signing-key" `isPrefixOf` filename) $ Just <$> readVerificationKey @w fullPath
                  ".skey" -> if ("verification-key" `isPrefixOf` filename) $ Just <$> readSigningKey @w fullPath
                  _ -> pure Nothing
        )
        files
