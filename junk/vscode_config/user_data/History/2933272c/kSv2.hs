...
import Ledger.Scripts (Datum, MintingPolicy (MintingPolicy, getMintingPolicy), MintingPolicyHash (MintingPolicyHash), Script,
...
addMintingPolicy vvl rd tx@Tx{txMintingScripts, txScripts} = tx
    ...
    where
        ...
        vl@(MintingPolicy script) = unversioned vvl
