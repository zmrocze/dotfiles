...
import BlockType (BlockReference, TXID, BlockHeader, coinbaseOutputs, outputs, Output (Output), PublicAddress, Cent)
...
insertOwnedKeys txid vout pub priv (Cent cents) addr = Session.statement (txid, vout, pub, priv, (Cent), addr) Statement.insertOwnedKeys
