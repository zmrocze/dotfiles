...
cardanoMainnetAddress (BpiWallet _ pay stake) =
  CAPI.toAddressAny $
    CAPI.ShelleyAddress
      Ledger.Mainnet
      (Shelley.KeyHashObj kh $ (\case (CAPI.PaymentKeyHash kh) -> kh) . CAPI.verificationKeyHash $ vKey pay)
      (toShelleyStakeReference scr)
  
  where

    vKeyHashToPaymentCredential :: -> Shelley.PaymentCredential Shelley.StandardCrypto
