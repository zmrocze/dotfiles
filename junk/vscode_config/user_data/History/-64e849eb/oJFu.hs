...
withFundedCluster, startFundedCluster) where
...
withFundedCluster :: PlutipConfig -> [[Positive]] -> (ClusterEnv -> [KeyPair] -> IO a) -> IO a
withFundedCluster conf distribution action = withLocalCluster conf $ \cenv -> do
...
keepRunning ::
  ((env -> IO ()) -> IO ())
  -> (env -> IO b) -> IO (TVar (ClusterStatus b), b)
...
startFundedCluster conf distribution action = keepRunning (withFundedCluster conf distribution . curry) (uncurry action)
...
startCluster ::
  ...
  (ClusterEnv -> IO a) ->
  IO (TVar (ClusterStatus a), a)
startCluster conf = keepRunning (withLocalCluster conf)
