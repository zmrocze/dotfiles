...
-- keepRunning :: ((a -> IO b) -> IO b) -> ((a -> IO b) -> IO (TVar (ClusterStatus b), b))
keepRunning :: (MonadUnliftIO m1, Control.Monad.IO.Class.MonadIO m2) =>
  ((t -> m2 ()) -> m1 ())
  -> (t -> m2 b) -> m1 (TVar (ClusterStatus b), b)
keepRunning withSomething = \action -> do
  status <- newTVarIO ClusterStarting
  tid <- myThreadId
  void $
    forkFinally
      ( withSomething $ \env -> do
          b <- action env
