...
Set new `network.name` and `network.magic` in `runtime.nix`. Point to a correct block/slot`datumCache.blockFetcher.firstBlock` should be . Also see [Changing network configurations](./runtime.md#changing-network-configurations)
