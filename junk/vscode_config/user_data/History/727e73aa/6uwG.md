...
Set new `network.name` and `network.magic` in `runtime.nix`  and `datumCache.blockFetcher.firstBlock` should be pointed to a correct block/slot. Also see [Changing network configurations](./runtime.md#changing-network-configurations)
