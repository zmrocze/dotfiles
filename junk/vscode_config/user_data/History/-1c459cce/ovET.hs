...
import System.IO (IOMode (WriteMode), hClose, openFile, stdout, stderr, hPutStrLn)
...
withPlutusInterface conf action = do
  ...
  if False then retryClusterFailedStartup (return ()) else (return ())
  ...
  where
    ...
    retryClusterFailedStartup = 
      let msg err = "Retrying cluster startup due to: " <> show err 
          shouldRetry = pure . \case 
            ProcessHasExited _ _ -> True
            _ -> False
      in recovering (limitRetries 5)
        [logRetries shouldRetry (\x y z -> hPutStrLn stderr $ msg y <> ". " <> defaultLogMsg x y z)] 
      . const
