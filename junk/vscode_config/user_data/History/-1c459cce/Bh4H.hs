...
import System.IO (IOMode (WriteMode), hClose, openFile, stdout, hPrint, stderr, hPutStrLn)
  ...
  where
    ...
    retryOnAddressAlreadyInUseException = let msg = "Retrying cluster startup due to used ports: " in
      recovering (limitRetries 5)
        [logRetries (pure . isAlreadyInUseError) (\x y z -> hPutStrLn stderr $ msg <> defaultLogMsg x y z)] 
      . const
