...
withPlutusInterface conf action = do
  ...
  where
    ...
    -- 
    retryOnAddressAlreadyInUseException = let msg err = "Retrying cluster startup due to: " <> err  in
      recovering (limitRetries 5)
        [logRetries (\err@(ProcessHasExited _ _) -> pure True) (\x y z -> hPutStrLn stderr $ msg <> defaultLogMsg x y z)] 
      . const
