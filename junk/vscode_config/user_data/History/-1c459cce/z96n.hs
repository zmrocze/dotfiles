...
import System.IO (IOMode (WriteMode), hClose, openFile, stdout, hPrint, stderr)
...
withPlutusInterface conf action = do
  ...
  where
    ...
    -- This causes rare failure at the cluster startup (the "resource busy (Address already in use)" error)
    -- Given this is rare and the problem root sits in cardano-wallet, lets simply retry the startup few times full of hope.
    retryOnAddressAlreadyInUseException = let msg = "Retrying cluster startup due to used ports: " in
      recovering (limitRetries 5)
        [logRetries (pure . isAlreadyInUseError) (\x y z -> hPrint stderr $ msg <> defaultLogMsg x y z)] 
      . const
