...
main = do
  ...
  case totalAmount config of
    ...
    Right amt -> do
      ...
      (st, _) <- startFundedCluster plutipConfig (replicate numWallets $ replicate numUtxos amt) $ \cenv keys -> do
        for_ keys (saveKeyPair dirWallets)
  ...
  where
    ...
    printWallet (w, n) = do
      ...
      putStrLn $ "Wallet " ++ show n ++ " mainnet address: " ++ mkMainnetAddress w
