...
instance FromJSON TxMetadataValue where
    parseJSON =
        ...
        <|> withScientific "TxMetaNumber" (either (fail "Expected Integer got Float.") (fmap TxMetaNumber) . floatingOrInteger )
        ...
        where 
            ...
            parseList = withObject "TxMetaMap" (\o -> TxMetaList <$> o .: "list")
            parseNumber = withScientific "TxMetaNumber" (either (fail "Expected Integer got Float.") (fmap TxMetaNumber) . floatingOrInteger )
            parseBytes = withObject "TxMetaMap" (\o -> TxMetaBytes <$> (o .: "bytes" >>= decodeByteString))
            parseText = withObject "TxMetaMap" (\o -> TxMetaText <$> o .: "text")
