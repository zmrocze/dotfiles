...
import Data.Aeson (ToJSON (toJSON), FromJSON (parseJSON), KeyValue ((.=)), object, pairs, withObject, (.:), json')
...
instance FromJSON TxMetadataValue where
    parseJSON = withObject "TxMetaMap" ( \o ->
        TxMetaMap <$> o .: "map" 
        <|> TxMetaList <$> o .: "list"
        <|> TxMetaNumber <$> parseJSON o
        <|> TxMetaBytes <$> (o .: "bytes" >>= decodeByteString)
        <|> TxMetaText <$> o .: "text")
