...
buildTx :: TxConstr' a b -> (ScriptLookups -> a) -> (ScriptLookups -> b)
...
buildTx (MustBeSignedBy' pubKeyH) a sl = \case
    ...
    Just (pubkey,signature) ->
        if pubKeyHash pubkey == pubKeyH then 
            a $ _ 
        else 
            _
buildTx (MustPayToOtherScript' validatorHash mStakeHash datum value) = _
