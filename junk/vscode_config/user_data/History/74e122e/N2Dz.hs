...
import Test.Plutip.Contract.Types (WalletInfo(ownPaymentPubKeyHash, ownAddress))
...
main = do
  ...
  where
    ...
    printWallet (w, n) = do
      putStrLn $ "Wallet " ++ show n ++ " PKH: " ++ show (ownPaymentPubKeyHash w)
      putStrLn $ "Wallet " ++ show n ++ " mainnet address: " ++ show (ownAddress w)
