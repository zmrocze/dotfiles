  ...
  rule token = parse
      ...
      | integer as n {
        (* cringe *)
          ...
          in match (try Some (int_of_string n) with e -> None) with
            | Some i when i <= pow 2 31 - 1 -> INTEGER (Int32.of_int i)
      ...
      (* string literal *)
      | '"' {
        let rec takeUntil f lexbuf str = match f lexbuf with
          | None -> str
