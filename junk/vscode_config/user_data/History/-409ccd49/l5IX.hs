...
import Test.Plutip.Internal.BotPlutusInterface.Types (PkhWallet (PkhWallet), ValueOrdering (VLt), BaseWallet (BaseWallet))
...
walletLookupsTest :: Test.Plutip.Contract.TestRunner
  ghc-prim-0.6.1:GHC.Types.Any ghc-prim-0.6.1:GHC.Types.Any Char ()
walletLookupsTest = withContract $ \wl -> do
  ...
  case addrb of 
    Address (PubKeyCredential pkhb') (Just (StakingHash (PubKeyCredential spkhb'))) ->
      if pkhb == PaymentPubKeyHash pkhb' && spkhb == StakePubKeyHash spkhb' then pure ()
  ...
  case addrc of 
    Address (PubKeyCredential pkhc') Nothing ->
      if pkhc == PaymentPubKeyHash pkhc' then pure ()
