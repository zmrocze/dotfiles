...
getInput outRef tx = do
    r <- listToMaybe $ mapMaybe (takeRedeemer . Tx.txInputType) $ filter (\Tx.TxInput{Tx.txInputRef} -> outRef == txInputRef) $ 
            _citxInputs tx
    PlutusTx.fromBuiltinData r
    where
        takeRedeemer (Tx.TxConsumeScriptAddress (Ledger.Redeemer r) _ _) = Just r
