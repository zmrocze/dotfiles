...
getInput outRef tx = do
    (_validator, Ledger.Redeemer r, _) <- listToMaybe $ mapMaybe _ $ filter (\Tx.TxInput{Tx.txInputRef, txInputType} -> outRef == txInRef && isScript txInputType) $ Set.toList $ _citxInputs tx
    PlutusTx.fromBuiltinData r
    where
        isScript TxConsumeScriptAddress {} = True
        isScript _ = False
getStates
    :: forall s i
