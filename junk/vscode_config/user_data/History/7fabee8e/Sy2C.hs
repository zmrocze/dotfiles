...
import BlockType (TXID, BlockReference, Transaction, BlockHeader (BlockHeader), PublicAddress, Cent)
...
insertOwnedKeys :: Statement (TXID , Int32, RSA.PublicKey, RSA.PrivateKey, Int32, PublicAddress) ()
insertOwnedKeys = Statement sql e D.noResult True
    where
        sql = "insert into owned_keys (keys_tx_id, vout, pub_key, priv_key, amount, pub_addr) values ($1, $2, $3, $4, $5, $6)"
        e = contrazip6 encodeHash encodeInt32 encodeBinary encodeBinary encodeInt32 encodeHash
        encodeInt32 = E.param $ E.nonNullable E.int4
...
selectOwnedUtxos :: Statement () (Vector (TXID, Int32, RSA.PublicKey, RSA.PrivateKey, Int32, PublicAddress))
selectOwnedUtxos = Statement sql E.noParams (D.rowVector d) True
    where
        sql = "select tx_id, vout, pub_key, priv_key, amount, pub_addr \
        \ from owned_keys"
        nonNullableColumn = D.column . D.nonNullable
        d = (,,,,,) 
            <$> rowHash
