...
```haskell
import Plutip.LocalCluster (startCluster, stopCluster)
```
...
```haskell
import Plutip.BotPlutusInterface.Wallet (BpiWallet, addSomeWallet)
...
Once you have that, you can simply use `runContract` from `import Plutip.BotPlutusInterface.Run`:
