...
```haskell
import PlutipLocalCluster (startCluster, stopCluster)
```
...
```haskell
import PlutipBotPlutusInterface.Wallet (BpiWallet, addSomeWallet)
...
Once you have that, you can simply use `runContract` from `import PlutipBotPlutusInterface.Run`:
