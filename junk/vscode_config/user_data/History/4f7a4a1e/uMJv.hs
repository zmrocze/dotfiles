...
getTxOutDatum ::
  ...
  Ledger.TxOut ->
  Maybe d
getTxOutDatum _ (Ledger.TxOut _ (Ledger.TxOut _ _ Nothing)) = Nothing
getTxOutDatum _ (Ledger.TxOut tx' (Ledger.TxOut _ _ (Just datumHash))) =
    Map.lookup datumHash (Ledger.txData tx') >>= (Ledger.getDatum >>> fromBuiltinData @d)
