...
import Spec.Governance.Transactions
  ( CreateProposalResult,
    ...
    versionIncrementGovDatumUpgradeCompiled, executeProposalExploitSuccessfully
  )
...
import Prelude (Int, String, replicate, (*))
...
tests suiteOpts cfg =
                ...
                CDP.runOpenCDPPosition
                  ...
                  CDPParams.OpenPositionParam
                    { CDPParams.opAsset = iBTCTokenName,
                      CDPParams.opCollateralAmount = 1_000,
  ...
  where
    ...
    createTextProposal = do
      let cpParam =
            CreateProposalParam (Gov.TextProposal (mconcat $ replicate 32 "a"))
      runCreateProposal cpParam
    ...
    vote' (pId, endTime, totalShards) = do
      runCreateShards (CreateShardsParam pId endTime totalShards)
      waitNSlots 1
      runVote (VoteParam pId Poll.Yes)
