...
envelope :: TypedWriter pt => Text -> ClosedTerm pt -> App TypedScriptEnvelope
envelope nm pt = case mkEnvelope (Config {tracingMode = DoTracing}) nm pt of
  Left err -> throwError err
  Right env -> pure env

save_ :: TypedWriter pt => Text -> ClosedTerm pt -> App ()
save_ nm pt =  modify' $ M.insert nm env
...
runExporter exporter = do
  ...
  index <- foldM go M.1 (M.toList toWrite)
