...
type App = StateT (Map Text (TypedScriptEnvelope, HashStr)) (Either Text)
...
saveAux :: TypedWriter pt => Text -> ClosedTerm pt -> App (TypedScriptEnvelope, HashStr)
saveAux nm pt = case mkEnvelope (Config {tracingMode = DoTracing}) nm pt of
  Left err -> throwError err
  Right env -> (env, hashTypedEnvelope env)
