...
type App = StateT (Map Text (TypedScriptEnvelope, ScriptHash)) (Either Text)
...
saveAux :: TypedWriter pt => Text -> ClosedTerm pt -> App (TypedScriptEnvelope, ScriptHash)
saveAux nm pt = case mkEnvelope (Config {tracingMode = DoTracing}) nm pt of
  Left err -> throwError err
  Right env -> pure (env, hashTypedEnvelope env)

save_ :: TypedWriter pt => Text -> ClosedTerm pt -> App ()
save_ nm pt = do 
  scr <- saveAux nm pt 
  modify' $ M.insert nm scr
