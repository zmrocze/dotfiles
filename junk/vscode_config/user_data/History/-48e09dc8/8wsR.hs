...
runExporter dir exporter = do
  ...
  putStrLn $ "Writing " <> show (length toWrite) <> " scripts"
  let toWriteWithHashes = M.map (\env -> (env, hashTypedEnvelope env)) toWrite
  -- save plutus script files
  traverse_ saveScript $ M.toList toWriteWithHashes
  ...
  where
    ...
    saveScript (nm, (env, hash)) = do
      ...
      putStrLn $ "Writing " <> T.unpack nm <> " to " <> dir </> showHash hashStr <> ".plutus"
      writeEnvelope
        (dir </> hashStr <> ".plutus")
        env
