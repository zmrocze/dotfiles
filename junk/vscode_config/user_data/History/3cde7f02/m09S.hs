...
import PlutusPrelude (render, trace)
        ...
        DisplayOnlyFromContext logCtx logLvl ->
          filter
            ( \LogLine {logLineContext, logLineLevel} ->
                trace (show (logLineContext == logCtx
                  && sufficientLogLevel logLvl logLineLevel)) (logLineContext == logCtx
