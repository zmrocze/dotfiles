...
assertExecutionWith options tag testWallets testRunner predicates =
  ...
  where
    ...
    optionToTestTree BudgetCounting = singleTest "Budget stats" . StatsReport
    optionToTestTree Tracing = singleTest "BPI logs (PAB requests/responses)" . LogsReport DisplayAllTrace
    optionToTestTree (TracingButOnlyContext logCtx logLvl) =
      singleTest "BPI logs (PAB requests/responses)" . LogsReport (DisplayOnlyFromContext logCtx logLvl)
