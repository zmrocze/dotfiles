...
import Ledger.Tx (CardanoTx, ChainIndexTxOut, SomeCardanoApiTx, Tx (txFee, txMint), TxIn (TxIn, txInRef, txInType), TxOut (TxOut), TxOutRef, TxInput (TxInput, txInputRef), TxInType (ConsumeScriptAddress, ConsumeSimpleScriptAddress, ConsumePublicKeyAddress), TxInputType (TxConsumeSimpleScriptAddress, TxConsumePublicKeyAddress))
...
handleBalanceTx utxo utx = do
    ...
    let fees = txFee filteredUnbalancedTxTx
        ...
        addTxIn tx TxIn {txInRef, txInType}  = case txInType of
            ConsumeScriptAddress pv val rd dt -> addScriptTxInput txInRef pv val rd dt tx
            ConsumeSimpleScriptAddress -> tx & over inputs ( TxInput txInRef TxConsumeSimpleScriptAddress :)
            ConsumePublicKeyAddress -> tx & over inputs ( TxInput txInRef TxConsumePublicKeyAddress :)
...
newtype PubKeyTxIn = PubKeyTxIn TxOutRef
