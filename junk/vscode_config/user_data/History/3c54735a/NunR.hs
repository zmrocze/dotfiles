...
import Ledger.Tx (CardanoTx, ChainIndexTxOut, SomeCardanoApiTx, Tx (txFee, txMint), TxIn (TxIn, txInRef, txInType), TxOut (TxOut), TxOutRef, TxInput (TxInput, txInputRef), TxInType (ConsumeScriptAddress, ConsumeSimpleScriptAddress, ConsumePublicKeyAddress))
...
handleBalanceTx utxo utx = do
    ...
    let fees = txFee filteredUnbalancedTxTx
        ...
        addTxIn tx TxIn {txInRef, txInType}  = case txInType of
            ...
            ConsumePublicKeyAddress -> tx & over inputs ( TxInput txInRef TxConsumeSimpleScriptAddress :)
