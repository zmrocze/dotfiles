...
import Control.Lens (At (at), makeLensesFor, over, view, (%=), (.=), (<>=))
...
import Ledger.Tx (ChainIndexTxOut, Tx, TxInput (TxInput), TxInputType (TxConsumePublicKeyAddress),
                  TxOut (txOutAddress, txOutDatumHash, txOutValue), TxOutRef, addScriptTxInput, txInRef, TxIn (TxIn), TxInType(ConsumeScriptAddress))
...
addOwnInput ScriptInputConstraint{icRedeemer, icTxOutRef} = do
    ...
    -- let (TxIn outRef (Just (ConsumeScriptAddres vl rs dt))) = txIn
    -- unbalancedTx . tx . Tx.inputs %=  (: scriptTxIn2TxInput (Typed.tyTxInTxIn txIn))
    case Typed.tyTxInTxIn txIn of 
        -- this is what makeTypedScriptTxIn makes
        TxIn outRef (Just (ConsumeScriptAddress vl rs dt)) -> do
            unbalancedTx . tx %= addScriptTxInput outRef vl rs dt
...
processConstraint = \case
    ...
    MustSpendPubKeyOutput txo -> do
        ...
        case txout of
          Tx.PublicKeyChainIndexTxOut { Tx._ciTxOutValue } -> do
              ...
              unbalancedTx . tx . Tx.inputs %= (TxInput txo TxConsumePublicKeyAddress :)
    ...
    MustSpendScriptOutput txo red -> do
        ...
        case mscriptTXO of
          Just (validator, datum, value) -> do
            unbalancedTx . tx %= addScriptTxInput txo validator red datum
    ...
    MustMintValue mpsHash@(MintingPolicyHash mpsHashBytes) red tn i -> do
        ...
        unbalancedTx . tx . Tx.scriptWitnesses %= Map.insert (ScriptHash mpsHashBytes) mintingPolicyScript
