...
import Prelude
import BotPlutusInterface.ChainIndex (removeCollateralFromPage)
  ...
  UtxoSetAtAddress pageQuery _ -> do
    ...
    pure $
      UtxoSetAtResponse $
        UtxosResponse
          (state ^. tip)
          (removeCollateral (Just $ _collateralUtxo state) $ pageOf pageQuery (Set.fromList (state ^. utxos ^.. traverse . _1)))
