...
instance Ord LogLevel where 
   msgLogLevel <= logLevelSetting =
    (  msgLogLvl `constrLEq` logLevelSetting
        && not (null intersectLogTypes)
    )
    where 
      intersectLogTypes = ltLogTypes logLevelSetting `intersect` (ltLogTypes msgLogLvl <> [AnyLog])

      constrLEq (Error _) _ = True
      constrLEq (Warn _) (Error _) = False
      constrLEq (Warn _) _ = True
      constrLEq (Notice _) (Error _) = False
      constrLEq (Notice _) (Warn _) = False
      constrLEq (Notice _) _ = True
