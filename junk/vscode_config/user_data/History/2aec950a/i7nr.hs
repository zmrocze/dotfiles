...
-- | Defined so that 
-- > msgLogLvl <= settingLogLevel
-- iff message should be displayed with this log level setting.
-- Not transitive.
sufficientLogLevel msgLogLvl logLevelSetting =
      msgLogLvl `constrLEq` logLevelSetting -- the log is important enough
      && not (null intersectLogTypes)   -- log is of type we're interested
