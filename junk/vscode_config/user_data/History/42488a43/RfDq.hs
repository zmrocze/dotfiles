...
import Plutus.Script.Utils.V1.Generators (alwaysSucceedPolicy, someTokenValue)
import Plutus.Script.Utils.V1.Generators qualified as ScriptGen
import Plutus.Script.Utils.V1.Scripts (scriptHash, validatorHash)
    ...
    if fee' < totalValAda
        then do
            ...
            let tx = mempty
                        ...
                        , txData = Map.fromList (map (\d -> (Scripts.datumHash d, d)) datums)
