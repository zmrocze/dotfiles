...
import Ledger (Ada, CardanoTx (EmulatorTx), CurrencySymbol, Interval, OnChainTx (Valid),
               ...
               PaymentPrivateKey (unPaymentPrivateKey), PaymentPubKey (PaymentPubKey), 
               ScriptContext (ScriptContext), Slot (Slot), SlotRange, SomeCardanoApiTx (SomeTx),
               TokenName, Tx (..), TxIn,
               TxInInfo (txInInfoOutRef),  TxInfo (TxInfo), TxInput (..),
               TxInputType (TxConsumePublicKeyAddress), TxOut (txOutValue), TxOutRef (TxOutRef), UtxoIndex (UtxoIndex),
               ValidationCtx (ValidationCtx), Value, _runValidation, addMintingPolicy,
               addSignature',  pubKeyTxOut, toPublicKey, txData, txId, txScripts)
import Plutus.V1.Ledger.Api (CurrencySymbol, Datum, Interval, MintingPolicy,
               POSIXTime (POSIXTime, getPOSIXTime), POSIXTimeRange,
               ScriptContext (ScriptContext),
               TokenName,
               TxInInfo (txInInfoOutRef), TxInfo (TxInfo), Validator (Validator), ValidatorHash)
...
genValidTransactionSpending' g feeCfg ins totalVal = do
    ...
    where
        ...
        txInToTxInput (TxInputWitnessed outref txInType) = case txInType of
            ...
            ConsumeScriptAddress vl rd dt -> (TxInput outref (Ledger.TxConsumeScriptAddress rd (plutusV1ValidatorHash vl) (datumHash dt)), Just (vl, dt))
