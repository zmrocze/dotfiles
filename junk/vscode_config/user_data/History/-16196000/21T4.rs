...
fn main () {
    ...
    let unit_js_decoded = PlutusData::decode_fragment(&unit_js).unwrap();
    let unit_js_encoded = unit_js.encode_fragment().unwrap();
    // I dont even care why ^ above is different to initial unit_js, roundtrip doesn't work. 
    // Because its so much different that the difference is bigger than the one in the resulting script hash. left for later
