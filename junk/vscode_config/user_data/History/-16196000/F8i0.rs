...
fn main () {
    let plutus_unit : PlutusData = PlutusData::Constr(Constr{tag: 127, any_constructor: None, fields: vec![]});
    ...
    let unit: [u8; 5] = [159, 192, 159, 255, 255];
    let arg_ba: [u8; 4] = [159, 4, 5, 255]; // [4,5]

    let applied_script = apply_params_to_script_with_errors(&unit, &script0a_bytes).unwrap();
    print!("applied\n");
