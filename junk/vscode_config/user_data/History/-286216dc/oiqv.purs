...
import Effect.Aff (Aff, Canceler(Canceler), fiberCanceler, launchAff, launchAff_, makeAff, throwError, try)
      ...
      { url, wallet: WalletExtension wallet } -> do
        ...
        withBrowser opts.noHeadless rt (Just extensionId) \browser -> do
          withE2ETest (wrap url) browser \re@{ page } -> do
            let
              confirmAccess =
                case wallet of
                  ...
                  GeroExt -> \_ _ -> throwError (error "OHH wghat ive done!")
            ...
            makeAff $ \k -> do
              let rethrow aff = launchAff_ $ do
                    res <- try aff
                    if isLeft res then 
                      (liftEffect $ k res)
                    else 
                      pure unit
              map fiberCanceler $ launchAff $ do
                (\x -> liftEffect <<< k =<< x) $ try $ subscribeToBrowserEvents (Just $ Seconds 10.0) page
                  case _ of
