...
import Control.Monad.Reader (MonadReader (ask), ReaderT (runReaderT), ask)
...
withConfiguredCluster ::
  PlutipConfig ->
  [[Positive]] ->
  ((ClusterEnv, [BpiWallet]) -> IO a) ->
  IO a
withConfiguredCluster conf twalls action = do
  startCluster conf (\cenv -> do
    wallets <- setup
    a <- action wallets
    stopCluster cenv
    pure a)
  ...
  where
    ...
    setup cenv = do
      wallets <- traverse (runReaderT addSomeWallet cenv) twalls
