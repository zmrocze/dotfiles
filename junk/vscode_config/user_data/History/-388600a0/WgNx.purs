...
import Data.Function.Uncurried (Fn1, mkFn1, Fn2, mkFn2, Fn3, mkFn3)
...
insertPWTXHash = mkFn3 $ \str txhash arr ->
  cons (wrap { password: str, txhash: txhash })
    <<< runFn2 deletePWTXHash str
    $ arr

deletePWTXHash :: Fn2 String (Array PWTXHash) (Array PWTXHash)
deletePWTXHash = mkFn2 $ \str arr -> filter (\x -> (unwrap x).password /= str) arr
...
burnTokens = \tkStr amt -> execContract $ burnTokens' tkStr amt
